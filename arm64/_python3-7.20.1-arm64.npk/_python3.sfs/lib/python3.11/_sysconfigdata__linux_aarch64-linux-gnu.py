...
build_time_vars = {
    ...
    "CONFIG_ARGS": "'--build=aarch64-unknown-linux-gnu' '--host=aarch64-unknown-linux-gnu' '--prefix=/install' '--with-openssl=/tools/deps' '--with-system-expat' '--with-system-libmpdec' '--without-ensurepip' '--with-readline=editline' '--enable-static-libpython-for-interpreter' '--enable-shared' '--enable-optimizations' '--with-lto' '--with-build-python=/tools/host/bin/python3.11' '--with-dbmliborder=bdb' 'build_alias=aarch64-unknown-linux-gnu' 'host_alias=aarch64-unknown-linux-gnu' 'PKG_CONFIG=pkg-config --static' 'PKG_CONFIG_PATH=/tools/deps/share/pkgconfig:/tools/deps/lib/pkgconfig' 'CC=clang' 'CFLAGS=  -fPIC  ' 'LDFLAGS=  -Wl,--exclude-libs,ALL' 'CPPFLAGS=  -fPIC  '",
    ...
    "PYTHONPATH": "",
    "PYTHON_FOR_BUILD": "_PYTHON_PROJECT_BASE=/build/Python-3.11.13 _PYTHON_HOST_PLATFORM=$(_PYTHON_HOST_PLATFORM) PYTHONPATH=$(shell test -f pybuilddir.txt && echo /build/Python-3.11.13/`cat pybuilddir.txt`:)./Lib _PYTHON_SYSCONFIGDATA_NAME=_sysconfigdata__linux_aarch64-linux-gnu /tools/host/bin/python3.11",
    ...
    "SETPGRP_HAVE_ARG": 0,
    "SHAREDMODS": "Modules/_crypt.cpython-311-aarch64-linux-gnu.so",
    "SHELL": "/bin/sh",
