...
return attrs.namevalAttr;};ObjectMap.prototype.toString=function(obj){return toString(this.getNameType(obj),obj);};ObjectMap.prototype.getName=function(obj){return getObjectName(obj);};ObjectMap.prototype.getFullName=function(obj){var i={name:this.attrs.name,cfg:getSysProps(this.attrs),propname:'title'};if(obj._type.nameval){if(obj.ufe0001!=null){i.propname='name';i.postfix='<'+this.toString(obj)+'>';}else{if(i.cfg&&i.cfg['name']){i.name='New '+i.cfg['name'];}else{i.name='New '+i.name;}
i.propname='newname';}}
...
var req={};req.Uff0001=this.attrs.path;req.uff0007=this.attrs.getallcmd||0xfe0004;req.ufe0018=0;req.ufe000c=0x20000000;post(req,rep=>{if(isError(rep))return;this.defconf=rep.mfe001d;if(this.defconf){delete this.defconf.bfe0008;const nameTag=obj._type?.namevalAttr?.id;if(nameTag)delete this.defconf[nameTag];updateOnNotification(obj,this.defconf);this.lstns.notify(obj);}else{this.defconf={};}});};function isRoTag(tag,owner){return owner.attrs.c.some(attr=>(attr.id===tag)&&attr.ro);}
function isFlag(tag,owner){return owner.attrs.c.some(attr=>(attr.id===tag)&&((attr.type==="flag")||(attr.type==="numflag")));}
ObjectMap.prototype.copyConf=function(obj){if(this.copySource){const nameType=obj._type?.namevalAttr?.type;const nameTag=(nameType==="string")?obj._type?.namevalAttr?.id:null;obj.Sfe001c??=[];for(const i in this.copySource){if((i===nameTag)||isRoTag(i,obj._owner)||isFlag(i,obj._owner)||(i.charAt(0)=='_')){continue;}
obj[i]=structuredClone(this.copySource[i]);}
delete this.copySource;delete obj.ufe0001;delete obj.bfe0007;delete obj.bfe0008;delete obj.bfe000d;}else{return this.setDefaultConf(obj);}};ObjectMap.prototype.fetch=function(obj){const req={};req.ufe0001=obj.ufe0001;req.Uff0001=this.attrs.path;req.uff0007=0xfe0002;post(req,rep=>{if(isError(rep))return;removeSysFields(rep);update(obj,rep);obj.ufe0001=req.ufe0001;this.lstns.notify(obj);});};ObjectMap.prototype.foreach=function(cb){if(this.attrs.ordered){var obj=this.last;while(obj!=null){cb(obj);obj=obj._prev;}}else{for(var i in this.map)cb(this.map[i]);}};ObjectMap.prototype.getFirst=function(){return this.first;};ObjectMap.prototype.getNext=function(obj){return obj._next;}
...
if(cb&&!cb(rep)){me.getallinprogress=false;}else if(isError(rep)){me.getallinprogress=false;if(rep.uff0008===0xfe0004){for(let i in me.map){const obj=me.map[i];--me.size;obj.ufe0013=1;if(attrs.ordered)me.unlink(obj);delete me.map[i];me.lstns.notify(obj);}
me.getall(cb);return;}else if(rep.uff0008==16646158){showError(`Failed to get all ${me.attrs.maxobjsmsg ?? ""}`,rep);}}else if((rep.ufe0003!=null||rep.mfe0015)&&!me.block){if(rep.ufe0003!=null)req.ufe0003=rep.ufe0003;if(rep.mfe0015!=null)req.mfe0015=rep.mfe0015;post(req,onreply);}else{for(let id in oldids){const obj=me.map[id];if(!obj)continue;--me.size;obj.ufe0013=1;if(attrs.ordered)me.unlink(obj);delete me.map[id];me.lstns.notify(obj);}
...
if(rep.ufe0001!=null)this.req.ufe0001=rep.ufe0001;this.rep.ufe0001=this.req.ufe0001;this.started=true;if(this.autorefresh){this.lstns.notify(this.rep);this.fetch();}else{this.stop();}});};ObjectAction.prototype.stop=function(){this.clearPrompt();if(this.started){this.started=false;if(this.timer){cancelTimer(this.timer);this.timer=null;}
const req={};req.Uff0001=this.attrs.path;req.uff0007=this.attrs.cancelcmd;if(this.req.ufe0001!=null)req.ufe0001=this.req.ufe0001;post(req,rep=>{this.lstns.notify(this.rep);});}};ObjectAction.prototype.fetch=function(){const req={};req.Uff0001=this.attrs.path;req.uff0007=this.attrs.pollcmd;if(this.req.ufe0001!=null)req.ufe0001=this.req.ufe0001;if(!this.promtInProgress&&this.confirmAnswer){req.mfe0028=this.confirmAnswer;delete this.confirmAnswer;}
...
if(this.autorefresh&&!this.timer){this.timer=timer(()=>{this.timer=null;this.fetch();},this.attrs.autorefresh);}});};ObjectAction.prototype.clearPrompt=function(){this.promptInstace?.close();}
ObjectAction.prototype.prompt=function(obj){if(this.promtInProgress){if(this.promtInProgress===obj.s1)return;this.clearPrompt();}
...
dialogManager.request(di=>{di.setTitle("Conformation needed");di.onsubmit(e=>{e.preventDefault();const checked=group.querySelector("input:checked");if(checked)this.confirmAnswer.s65=checked.value;di.close();di.onclose(null);});di.onclose(()=>{delete this.promtInProgress;this.timer=null;this.fetch();});di.setBody([fieldset]);this.promptInstace=di;});};ObjectAction.prototype.autostart=function(obj){this.start();return false;};ObjectAction.prototype.listen=function(cb,full){if(full)++this.autorefresh;this.lstns.listen(cb);};ObjectAction.prototype.unlisten=function(cb,full){if(full&&--this.autorefresh==0){this.stop();}
...
if(!this.next)return false;return this.next.cmp(o1,o2);};function CmpDescending(col,coldata,idx,next){this.col=col;this.coldata=coldata;this.idx=idx;this.next=next;}
...
if(this.comment)idx++;var first=idx;for(let i in this.columns){if(!this.columns[i])continue;const colIdx=this.attrs.columns.indexOf(this.columns[i].attrs);let attrs=type.columns[colIdx];let val=ftype(attrs).get(attrs,obj);attrs=this.attrs.columns[colIdx];const cell=line.cells[idx];let data=cell.firstChild;if(idx==first&&this.attrs.icons&&data)data=data.nextSibling;var d=ftype(attrs).cell(this.columns[i],obj,val,data);if((d&&!d.isEqualNode(data))||(d===null&&data!==null)){clearNodes(cell);if(idx==first){var icons=this.attrs.icons;if(obj._type.icons)icons=obj._type.icons;if(icons){var nr=0;if(this.iconon){nr=ftype(this.iconon).get(this.iconon,obj);if(nr==null||this.attrs.icons[nr]==null)nr=0;}
...
if(!this.newObj&&(type.type=="map")&&!type.nonaddable&&!type.ro&&!shouldHide(copyCfg)){this.copy=this.addActionButton("Copy",copyCfg);this.copy.onclick=e=>{this.obj._owner.copySource=this.obj;openContent(getPath(this.attrs)+".copy",true);}}
...
return cfg;}
const generalMenuCfg=[];function restructureSysmap(map){const arr=[];arr.push({name:"General Menu",type:"toolbar",node:{name:"General Menu"},entries:generalMenuCfg});arr.push({name:"Terminal",type:"button",node:{name:"Terminal"}});arr.push({name:"Advanced",type:"button",node:{name:"Advanced"}});for(const item of map){if(!isServiceVisible(item,true))continue;const cfg=restructureGeneric(item);cfg.type="service";const group=item.group;if(group){const grpItem=arr.find(e=>e.name===group&&e.type==="serviceGroup");if(!grpItem){arr.push({name:group,type:"serviceGroup",entries:[cfg],node:{name:group}});}else{grpItem.entries.push(cfg);}}else{arr.push(cfg);}}
...
function makeCheckbox(cfg,nameCtrl){const enable=make.input({p:nameCtrl},"checkbox");enable.checked=true;cfg.getAllChildren=()=>{const allChildren=[];cfg.children?.forEach(child=>{allChildren.push(child);});const buttons=cfg["&"];if(buttons){for(const btn in buttons){allChildren.push(buttons[btn]);}}
return allChildren;}
...
function generateMenuBar(){const topCfg=skin["General Menu"]??{};const undoButton=document.getElementById("undo-button");const redoButton=document.getElementById("redo-button");const unCfg=getProp(topCfg,undoButton.title);if(unCfg._hide)hide(undoButton);const updateUndoState=()=>{post({Uff0001:[17],Uff0002:[1],uff0007:0x80007},rep=>{if(isError(rep)){undoButton.disabled=true;if(!redoButton.disabled)redoButton.tabIndex=0;}else{undoButton.disabled=false;redoButton.tabIndex=-1;undoButton.title="Undo: "+rep.s3;undoButton.ariaDescription=undoButton.title;}});}
...
subscribe([17],rep=>{updateUndoState();updateRedoState();});updateUndoState();updateRedoState();const safeSwitch=document.getElementById("safe-mode");const safeCfg=getProp(topCfg,"Safe Mode");if(safeCfg._hide)hide(safeSwitch.closest("div.check-group"));const bold=safeSwitch.nextElementSibling?.querySelector("b");bold.innerText=viewLabel(safeCfg,"Safe Mode").innerText;safeSwitch.onchange=()=>{safeSwitch.checked=!safeSwitch.checked;toggleSafeMode();};function setButton(item){const text=item.lastElementChild.dataset.name;const cfg=getProp(skin,text);if(cfg._hide)hide(item);item.lastElementChild.textContent=viewLabel(cfg,text).textContent;item.onkeydown=e=>{if(e.key==="ArrowLeft")dom.focusPrevious(item);if(e.key==="ArrowRight")dom.focusNext(item);if(e.key==="Home")dom.focusFirst(item);if(e.key==="End")dom.focusLast(item);}}
