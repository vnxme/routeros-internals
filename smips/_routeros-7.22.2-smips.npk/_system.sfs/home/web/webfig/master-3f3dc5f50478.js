...
getSubtypesFor(gen,types){let subtypes=0;for(const i in gen.subtypes){const type=gen.subtypes[i];if(type.generic)subtypes+=this.getSubtypesFor(type,types);if(type.nonaddable||(type.pred&&!isTrue(type.pred)))continue;types.push(type.title);++subtypes;}
...
let msg=size!=1?size+" items":"1 item";const count=this.map.getCount();if(count!=null&&size!=count){msg+=" out of "+count;if(size===0){this.splash.filter();}else{this.splash.hide();}}
...
return false;}
Table.prototype.shouldHide=function(obj){if(!this.filter)return false;if(this.filter.value<0){if(this.filter.pred){let val=ftype(this.filter.attr).get(this.filter.attr,obj);return!isTrue(this.filter.pred,val);}
return false;}
