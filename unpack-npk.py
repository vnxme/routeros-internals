...
def unpack_npk_package(package: NovaPackage, directory: str, skip_files: bool = False, skip_squashfs: bool = False):
    ...
    if (not skip_squashfs) and (NpkPartID.SQUASHFS in package) and (len(package[NpkPartID.SQUASHFS].data) > 0):
        open(f'{directory}/{package_name}.sfs', 'wb').write(package[NpkPartID.SQUASHFS].data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip-files', type=bool, action='store_true', help='skip files')
    parser.add_argument('--skip-squashfs', type=bool, action='store_true', help='skip squashfs')
