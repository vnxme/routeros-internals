...
return path;}
getAttrCfg(attr){function hasAttr(a){if(a?.id===attr?.id)return true;return!!a.c?.find(hasAttr);}
...
return null;}
load(obj){if(!this.attrs.oncommited){if(!this.view){this.view=this.viewCtrl.getView(this.attrs.selon);this.lstn=()=>this.selectPane();this.view.listen(this.lstn);}}else{if(this.attrs.selon){this.onAttr=getAttr(obj._type,this.attrs.selon);}}
...
selectPane(obj){let p;if(this.attrs.oncommited){if(this.onAttr){p=this.findPane(ftype(this.onAttr).get(this.onAttr,obj));}else{for(let i in this.panes){const pane=this.panes[i];if(pane.on.isTrue(obj)){p=pane;break;}}}}else{p=this.findPane(this.view.getValue()||0);}
