...
function copyIdTags(dst,src){if(src?.ufe0001!=null){dst.ufe0001=src.ufe0001;}else if(src?.mfe0026!=null){dst.mfe0026=src.mfe0026;}}
function msg2hex(msg){return Array.from(msg2buffer(msg)).map(x=>x.toString(16)).join("");}
function assignId(obj){if((obj.ufe0001!=null)&&(obj.ufe0001!=0xffffffff)){obj._id=obj.ufe0001?.toString();}else if((obj.mfe0026!=null)){obj._id=msg2hex(obj.mfe0026);}}
...
static focusNext(node){const list=this.getAllInteractive(node.closest("ul, nav"));const idx=list.findIndex(el=>node.contains(el));list[(idx+1)%list.length]?.focus();}
static focusPrevious(node){const list=this.getAllInteractive(node.closest("ul, nav"));const idx=list.findIndex(el=>node.contains(el));list[(idx-1+list.length)%list.length]?.focus();}
static focusFirst(node){this.getAllInteractive(node.closest("ul, nav"))[0]?.focus();}
static focusLast(node){const l=this.getAllInteractive(node.closest("ul, nav"));l[l.length-1]?.focus();}
static addVerticalNav(list){list.addEventListener("keydown",(e)=>{switch(e.key){case"ArrowDown":dom.focusNext(e.target);break;case"ArrowUp":dom.focusPrevious(e.target);break;case"Home":dom.focusFirst(e.target);break;case"End":dom.focusLast(e.target);break;default:return;}
e.preventDefault();});}
static addHorizontalNav(list){list.addEventListener("keydown",(e)=>{switch(e.key){case"ArrowRight":dom.focusNext(e.target);break;case"ArrowLeft":dom.focusPrevious(e.target);break;case"Home":dom.focusFirst(e.target);break;case"End":dom.focusLast(e.target);break;default:return;}
e.preventDefault();});}}
...
function nullish(a){return a==null;}
function exists(a){return a!=null;}
...
class DialogManager{#title;#body;#form;buttons;constructor(){this.holder=get("exclusive-popup");this.#title=dom.getByTag(this.holder,"h2");this.#body=dom.getByTag(this.holder,"section");this.#form=dom.getByTag(this.holder,"form");this.buttons=get("exclusive-popup-buttons");this.queue=[];}
request(cb){if(!this.busy()){cb(new DialogInstance(this.holder,this.#title,this.#body,this.#form,this));this.holder.showModal();}
else this.queue.push(cb);}
release(){this.#title.replaceChildren();this.#body.replaceChildren();this.buttons.hidden=true;if(this.busy())this.holder.close();const cb=this.queue.shift();if(cb){cb(new DialogInstance(this.holder,this.#title,this.#body,this.#form,this));this.holder.showModal();}}
busy(){return this.holder.open;}}
const dialogManager=new DialogManager;class DialogInstance{#title;#form;#body;#valid;#mgr;#onclose;constructor(holder,title,body,form,mgr){this.#valid=true;this.#title=title;this.#body=body;this.#mgr=mgr;this.#onclose=null;this.#form=form;holder.onclose=()=>{this.close();}}
...
onsubmit(cb){this.#mgr.buttons.hidden=false;this.#form.onsubmit=cb;}
onclose(cb){this.#onclose=cb;}
close(){if(this.#onclose)this.#onclose();this.#valid=false;this.#mgr.release();}}
...
listenOn(obj){if(!obj._owner)return;const lstn=(o)=>{if(obj._id==o._id)this.update(obj);};obj._owner.listen(lstn,true);this.lstns.push({obj:obj,lstn:lstn});}
...
listen(l,arg){this.#lstns.push(l);return this.#lstns.length===1;}
unlisten(l,arg){for(let i in this.#lstns){if(this.#lstns[i]==l){this.#lstns.splice(Number(i),1);return this.#lstns.length===0;}}
return false;}
...
createRow(viewCtrl,cfg,obj){const label=new Label(viewLabel(cfg,this.attrs.title??this.attrs.name));viewCtrl.addRow(label,null,this);}
...
if(this.attrs.type=="doit"){b.onclick=e=>{if(!this.attrs.confirm||confirm(this.attrs.confirm)){let type=this.obj._type;while(type.owner)type=type.owner;const doit=new Doit(this.attrs,type.path);if(this.obj)doit.setID(this.obj);doit.doit();}};return;}
...
createRow(viewCtrl,cfg){const label=new Label(viewLabel(cfg,this.attrs.title??this.attrs.name));viewCtrl.addRow(label,this.createButton(),this);}
...
this.ctrl=c;if(this.bttn==null){c.style.display="inline";const r=make.span();r.style.display="flex";r.appendChild(this.createButton());r.appendChild(c);return r;}
return c;}
...
if(this.attrs?.optval===val)this.showCurtain();else if((obj[this.attrs.id]!=null)&&!obj[this.attrs.id])this.showCurtain();else if((obj[this.attrs.id]==null)&&this.attrs.def&&this.attrs.def!=BADID)this.hideCurtain();else this.showCurtain();}
...
this.blocked=false;}
fillList(obj,filter,cb){const values=this.attrs.values;const type=enm[values.type];const sort=!this.attrs.sortbyvalue;let listIsBroken=false;const newOpt=id=>{const opt=make.el("li",{cont:type.toString(id,values,obj)});opt.role="option";opt.dataset.id=id;opt.id=`dropdown-${this.fld.id}-option-${id}`;const less=(a,b)=>a.textContent<b.textContent;const idx=sort?binarySearch(this.list.children,opt,less):this.list.childElementCount;this.list.insertBefore(opt,this.list.childNodes[idx]);if(this.list.childElementCount>500){listIsBroken=true;this.list.style.outline="red solid 1px";const warning=make.div({cont:"!! List is incomplete !!"})
warning.style.textAlign="center";warning.style.fontWeight="bold";this.list.prepend(warning);}
return idx;};const lstn=(nobj,more)=>{const id=nobj.ufe0001??0xffffffff;const value=nobj.sfe0010||type.toString(id,values,obj);const li=Array.from(this.list.children).find(e=>e.textContent===value);if(nobj.ufe0013){if(li)this.list.removeChild(li);}else if(li){if(!filter(value))this.list.removeChild(li);else if(li.textContent!==value)li.textContent=value;}else{if(!filter(value))return;if(listIsBroken)return;if(cb)cb(id,newOpt(String(id)),more);}};const map=type.getMap(values,obj);for(const i in map){if(listIsBroken)break;if(map[i]&&filter(map[i]))newOpt(i);}
return lstn;}
...
create(viewCtrl,label){this.label=label;this.ctrl=make.el("fieldset",{cls:"f-row"});this.ctrl.id=uniqueId.form(this.attrs);make.span({p:this.ctrl,cls:"check-group"});return this.ctrl;}
...
const uniqueId=`${this.attrs.id}-${id}`;const holder=make.span({p:this.ctrl.firstElementChild,cls:"check"});const btn=make.input({p:holder},"radio");btn.name=this.id;btn.id=uniqueId;make.label({p:holder,cont:name},uniqueId);btn.onchange=()=>{if(!this.updating){this.label.setChanged();this.updateValue();this.notify();}};this.radios[id]=btn;}}
...
class SetView extends View{constructor(attrs,ro){super(attrs);this.boxes={};this.items=0;this.disabled=ro;this.ctrl=make.el("fieldset",{cls:"f-row"});this.ctrl.id=uniqueId.form(this.attrs);this.group=make.span({p:this.ctrl,cls:"check-group"});}
create(viewCtrl,label){this.label=label;this.viewCtrl=viewCtrl;return this.ctrl;}
getValue(){let set=0;for(let i in this.boxes){if(this.boxes[i].checked)set|=1<<parseInt(i);}
return set;}
load(obj){this.findLimit(obj);this.obj=obj;if(!this.lstn){const values=this.attrs.values;const type=enm[values.type];this.lstn=(obj)=>{if(!this.boxes[obj._id])this.addItem(obj._id,obj);};type.listen(values,this.lstn,this.viewCtrl);var map=type.getMap(values,obj);for(let i in map){if(map[i]!="")this.addItem(i,obj);}}
...
this.items=0;if(this.ctrl){clearNodes(this.group);const values=this.attrs.values;const map=enm[values.type].getMap(values,this.obj);for(let i in map){if(map[i]!="")this.addItem(i,this.obj);}}}
destroy(){if(this.lstn){enm[this.attrs.values.type].unlisten(this.attrs.values,this.lstn);}}
...
if(!this.attrs.small){this.group.style.setProperty("--columns",this.attrs.columns||2);}
const holder=make.span({p:this.group,cls:"check"});const uniqueId=`${this.attrs.id}-${id}`;const box=make.input({p:holder},"checkbox");box.id=uniqueId;make.label({p:holder,cont:str},uniqueId);box.checked=this.value&&((this.value[0]&(1<<id))!=0);if(this.disabled)box.disabled=true;box.onchange=()=>{this.label.setChanged();this.notify();};this.boxes[id]=box;++this.items;}}
class BoolView extends View{constructor(attrs,ro){super(attrs);this.ro=ro;this.disabled=false;}
create(viewCtrl,label){this.label=label;if(!this.ctrl){this.ctrl=make.fRow();this.chk=make.input({p:make.span({p:this.ctrl,cls:"check-group"})},"checkbox");this.chk.id=uniqueId.form(this.attrs);this.chk.onchange=()=>{this.label.setChanged();this.notify();};if(this.ro||this.disabled)this.chk.disabled=true;}
...
createRow(viewCtrl,cfg,obj){if(this.ro)return View.prototype.createRow.call(this,viewCtrl,cfg,obj);const label=new Label(viewLabel(cfg,this.attrs.title??this.attrs.name));viewCtrl.addRow(label,-1,this);}
...
createRow(viewCtrl,cfg,obj){this.attrs.c.forEach(a=>{const view=viewCtrl.newView(obj,a,false,this.isRO()||a.ro);if(view){view.attach(viewCtrl,obj,cfg);this.views.push(view);}});if(this.views.length==0)return;this.label=null;if(this.attrs.name){this.label=new Label(viewLabel(cfg,this.attrs.title??this.attrs.name));}
...
load(obj){this.reqId={_id:obj._id,ufe0001:obj.ufe0001,mfe0026:obj.mfe0026};if((this.reqId._id!=null)||obj._type.type==="item"){this.fetch(obj._type.path);}}
destroy(){if(this.timer!=null)cancelTimer(this.timer);this.dead=true;}
fetch(path){if(this.onreply)return;const req={};req.Uff0001=this.attrs.path||path;req.uff0007=this.attrs.cmd||0xfe0010;copyIdTags(req,this.reqId);this.onreply=rep=>{if(this.dead)return;if(rep.ufe0003){req.ufe0003=rep.ufe0003;post(req,this.onreply);}else{if(this.timer==null){this.timer=timer(()=>{this.onreply=undefined;this.timer=null;this.fetch(path);},this.attrs.autorefresh||2000);}}
...
createRow(viewCtrl,cfg,obj){this.bttn=make.button({cls:"btn btn-down expanded"});this.bttn.ariaPressed="false";this.bttn.onclick=()=>{this.showGroup(!this.open);this.changed=true;};this.label=new Label(viewLabel(cfg,this.attrs.title??this.attrs.name));viewCtrl.addRow(this.label,this.bttn,this);}
...
createRow(viewCtrl,cfg,obj){const sb=get("statusbar");if(obj._owner.getType().type=="query"){this.ctrl=make.div({p:sb,cls:"f-item"});const id=`sb-${this.attrs.id}`;make.label({p:this.ctrl,cont:this.attrs.name},id);make.div({p:this.ctrl,cls:"f-row rovalue"}).id=id;return;}
this.ctrl=make.span({p:sb});}
load(obj){if(!this.ctrl)return;const attrs=this.attrs;let str=toString(attrs,obj);if(obj._owner.getType().type=="query"){replaceText(get(`sb-${this.attrs.id}`),`${str} ${attrs.postfix || ""}`.trim());return;}
...
createTable(obj){if(this.rows)return;const type={owner:obj._type};const header=make.tr({p:make.el("thead",{p:this.table})});const addCol=(col,attrs)=>{if(col&&col.width&&col.width!=0xffffffff){const th=make.th({p:header,cont:col.name});setCellWidth(th,col.width);col.attrs=attrs;this.columns?.push(col);}};let idx=0;this.columns=[];this.attrs.c.forEach(a=>{if((a.type==="deck")&&!a.nonpublic){const colArr=ftype(a).columns(a,this,type);colArr.forEach(col=>{addCol(col,a);});}else{const col=!a.nonpublic?ftype(a).column(a,this,type):null;addCol(col,a);}
...
if(obj._id==null)return;row.classList.add("selected");this.selectedRow=row;this.selected=obj._id;this.updateViews(obj);return false;};}
updateRow(obj){let row=this.rows[obj._id];if(obj.ufe0013){if(row){this.table.removeChild(row.parentNode);delete this.rows[obj._id];}
return;}
if(!row){row=make.tr({p:make.el("tbody",{p:this.table})});this.rows[obj._id]=row;if(this.flags)make.td({p:row});this.columns?.filter(v=>v).forEach(()=>make.td({p:row}));}
...
let col=this.flags?1:0;this.columns?.filter(v=>v).forEach((c,i)=>{const val=ftype(c.attrs).get(c.attrs,obj);const cell=row.cells[col];const d=ftype(c.attrs).cell(c,obj,val,cell.firstChild);if(d!=cell.firstChild){clearNodes(cell);if(d)cell.appendChild(d);}
++col;});if(this.attrs.views)this.attachHandler(row.parentNode,obj);if(this.selected!=null&&this.selected==obj._id){this.selectedRow=row.parentNode;this.updateViews(obj);}}
...
create(){const height=this.attrs.height||144;const g=createGraphic(null,2176,height,340,height,"graph");if(!g||!g.viewBox)return null;this.g=g;this.width=g.viewBox.width;this.height=g.viewBox.height;g.canvas.style.height=this.height;g.canvas.classList.add("graph-svg");g.strokeColor="#808080";g.strokeWidth=1;g.strokeWidth=2;this.curves.forEach((c,i)=>{g.strokeColor="#000";g.strokeOpacity=0;g.fillOpacity=0.3;g.fillColor=c.color;c.polygon=g.polygon(i-0?"secondary":"primary");g.strokeColor=c.color;g.strokeOpacity=0.4;g.fillOpacity=0;c.polyline=g.polyline(i-0?"secondary":"primary");});const level=(this.height-14)/5;for(let i=1;i<6;++i){const y=level*i-0.5;g.line(0,y,this.width,y);}
...
sattr=getAttr(src._type,links[i].src);val=ftype(sattr).get(sattr,src);}else{if(src.ufe0001){val=src.ufe0001;}else if(src.mfe0026){val=src.mfe0026;}
sattr=null;}
...
function stringToBytes(str){if(str.length==0)return null;const WHOLENUM=0;const DECIMALNUM=1;const EMPTY_OR_SCALE=2;const POSTFIX=3;var scale=1;var currStep=WHOLENUM;var wholePart="";var decimalPart="";for(var i=0;i<str.length;++i){var c=str.substr(i,1).toLowerCase();if(currStep==WHOLENUM){if(!isNaN(parseInt(c,10))){wholePart+=c;continue;}else if((c=='.')||(c==',')){currStep=DECIMALNUM;continue;}else{currStep=EMPTY_OR_SCALE;}}else if(currStep==DECIMALNUM){if(!isNaN(parseInt(c,10))){decimalPart+=c;continue;}else{currStep=EMPTY_OR_SCALE;}}
if((currStep==WHOLENUM)||(currStep==EMPTY_OR_SCALE)){switch(c){case' ':continue;case'b':scale=1;break;case'k':scale=1024;break;case'm':scale=1024*1024;break;case'g':scale=1024*1024*1024;break;default:return null;}
currStep=POSTFIX;}else if(currStep==POSTFIX){if((c==' ')||(c=='i')||(c=='b')){continue;}else{return null;}}}
var result=parseInt(wholePart,10)*scale;if(decimalPart.length){result+=parseFloat("."+decimalPart)*scale;}
...
if(attrs.cond&&attrs.cond.shouldHide())return null;var cfg=getAttrProp(type,attrs);if(shouldHide(cfg,attrs.hidebydefault))return null;var name=cfg.name||attrs.name;if(!attrs.inlinepostfix&&attrs.postfix){name+=' ('+attrs.postfix+')';}
return{name:name,width:attrs.width};};types.def.cell=function(col,obj,val){if(col.attrs.cond&&!col.attrs.cond.isTrue(obj))return null;let str=tostr(col.attrs,val)??"";if(str.length>0&&col.attrs.inlinepostfix)str+=col.attrs.postfix;return str?document.createTextNode(str):null;};types.def.listen=function(attrs,cb){if(attrs.values){var type=enm[attrs.values.type];if(type&&type.listen)type.listen(attrs.values,cb);}
...
return null;};types.def.alias=false;types.def.deferred=function(attrs){return false;};types.def.matcher=function(attr,opt,val){return null;};types.def.filters=function(attrs){return[];};types.bool=inherit(types.def);types.bool.get=function(attrs,obj){var val=obj[attrs.id||0];if(attrs.bit)val=val&attrs.bit?1:0;return attrs.inv?(val!=null?!val:null):val;};types.bool.put=function(attrs,obj,val){var b=attrs.bit||1;obj[attrs.id||0]=attrs.inv?(val?0:b):(val?b:0);};types.bool.fromstr=function(attrs,str){if(str=='yes'||str=='true')return 1;if(str=='no'||str=='false')return 0;return null;};types.bool.tostr=function(attrs,val){if(val==null)val=attrs.def||false;return val?'yes':'no';};types.bool.view=function(attrs,ro){return new BoolView(attrs,ro);};types.bool.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);if(typeVal==null)return null;switch(opt){case filterOptions.is:return function(v){return v==typeVal;}}
return null;};types.bool.filters=function(attrs){return[filterOptions.is];};types.number=inherit(types.def);types.number.get=function(attrs,obj){const val=obj[attrs.id||0]??attrs.def;return val instanceof Array?val[0]:val;};types.number.put=function(attrs,obj,val){if(val==null){val=attrs.optval!=null?attrs.optval:(attrs.def||0);}
obj[attrs.id||0]=val;};types.number.hasValue=function(attrs,val){if(attrs.optval!=null)return val!=attrs.optval;return val!=(attrs.def||0);};types.number.fromstr=function(attrs,str,limit){const n=lossyenum_fromstr(attrs,str,(attrs,str)=>{const n=string2int(str,attrs.radix||10);if(n==null||n<0)return null;if(!minmax(n,attrs.min,attrs.max??Number.MAX_SAFE_INTEGER))return null;return n;});if(n===null||!fitsRangeLimit(n,limit))return null;return n;};types.number.tostr=function(attrs,val){if(val==null){val=attrs.def||0;if(!minmax(val,attrs.min,attrs.max))val=attrs.min||0;}
...
str=this.tostr(attrs,attrs.min||0)+'..';if(attrs.max!=null)str+=this.tostr(attrs,attrs.max);return str;};types.number.addlimit=function(attrs,limit,str){if(!limit.ranges)limit.ranges=[];return parseLimit(limit.ranges,str,attrs.min||0,attrs.max||0xffffffff,function(str){return ftype(attrs).fromstr(attrs,str);});};types.number.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);if(typeVal==null)return null;switch(opt){case filterOptions.is:return function(v){return v==typeVal;}
case filterOptions.isNot:return function(v){return v!=typeVal;}
case filterOptions.less:return function(v){return v<typeVal;}
case filterOptions.lessEq:return function(v){return v<=typeVal;}
case filterOptions.greater:return function(v){return v>typeVal;}
case filterOptions.greaterEq:return function(v){return v>=typeVal;}}
return null;};types.number.filters=function(attrs){return[filterOptions.is,filterOptions.isNot,filterOptions.less,filterOptions.lessEq,filterOptions.greater,filterOptions.greaterEq];};types.bignumber=inherit(types.number);types.integer=inherit(types.number);types.integer.get=function(attrs,obj){var val=obj[attrs.id||0];return val!=null?num2int(val):null;};types.integer.put=function(attrs,obj,val){if(val==null){val=attrs.optval!=null?attrs.optval:(attrs.def||0);}
...
return res;};types.bytes=inherit(types.number);types.bytes.tostr=function(attrs,val){if(!val)return"0 B";val*=attrs.scale||1;if(val<5000)return val+" B";if(val<5000000)return(val/1024).toFixed(1)+" KiB";if(val<5000000000)return(val/(1024*1024)).toFixed(1)+" MiB";return(val/(1024*1024*1024)).toFixed(1)+" GiB";};types.bytes.fromstr=function(attrs,str,limit){const result=stringToBytes(str);return(result<=0xffffffff)?result:null;};types.bigbytes=inherit(types.bytes);types.bigbytes.fromstr=function(attrs,str,limit){return stringToBytes(str);}
types.bigbytes.tostr=function(attrs,val){if(!val)return"0 B";if(attrs.precise){const KBScale=1024;const MBScale=1024*KBScale;const GBScale=1024*MBScale;const TBScale=1024*GBScale;const tb=Math.trunc(val/TBScale);const gb=Math.trunc(val/GBScale);const mb=Math.trunc(val/MBScale);const kb=Math.trunc(val/KBScale);if(tb&&(!(val%TBScale)||!(val%(512*GBScale)))&&tb>=5){if(val%TBScale){return(val/TBScale).toFixed(1)+" TiB";}else{return tb+" TiB";}}else if(gb&&(!(val%GBScale)||!(val%(512*MBScale)))&&gb>=5){if(val%GBScale){return(val/GBScale).toFixed(1)+" GiB";}else{return gb+" GiB";}}else if(mb&&(!(val%MBScale)||!(val%(512*KBScale)))&&mb>=5){if(val%MBScale){return(val/MBScale).toFixed(1)+" MiB";}else{return mb+" MiB";}}else if(kb&&(!(val%KBScale)||!(val%512))&&kb>=5){if(val%KBScale){return(val/KBScale).toFixed(1)+" KiB";}else{return kb+" KiB";}}else{return val+" B";}}else{return types.bytes.tostr(attrs,val);}};types.bitrate=inherit(types.number);types.bitrate.tostr=function(attrs,val){if(!val)return"0 bps";if(val<2000&&val!=1000){return val+" bps";}
...
obj._timestamp[attrs.value]=[now,val,r];return r;};types.changerate.get=function(attrs,obj){return this.getRate(attrs,obj,attrs.scale||1);};types.changerate.tostr=types.decimal.tostr;types.changerate.view=function(attrs,ro){return types.def.view(attrs,1);};types.changerate.matcher=function(attr,opt,val){return types.decimal.matcher(attr,opt,val);};types.changerate.filters=function(attrs){return types.decimal.filters(attrs);};types.bigchangerate=inherit(types.changerate);types.bigchangerate.tostr=types.bigdecimal.tostr;types.bigbitchangerate=inherit(types.changerate);types.bigbitchangerate.get=function(attrs,obj){return this.getRate(attrs,obj,8);};types.bigbitchangerate.tostr=types.bigbitrate.tostr;types.fixedpoint=inherit(types.number);types.fixedpoint.tostr=function(attrs,val){var scale=attrs.scale||1;val=num2int(val||attrs.def||0);var minus=val<0?"-":"";var str=minus+Math.floor(Math.abs(val)/scale).toString()
...
return null;}
return[low,high];};types.integerrange.deflimit=function(attrs){let str='';if(attrs.low!=null)str=num2int(attrs.low).toString();str+='..';if(attrs.high!=null)str+=num2int(attrs.high).toString();return str;};types.integerrange.addlimit=function(attrs,limit,str){if(!limit.ranges)limit.ranges=[];return parseLimit(limit.ranges,str,attrs.low!=null?attrs.low:num2int(0x80000000),attrs.high!=null?attrs.high:num2int(0x7fffffff),function(str){var v=string2int(str);if(v==null||!iminmax(v,attrs.low,attrs.high))return null;return v;});};types.integerrange.matcher=function(attr,opt,val){var typeVal=types.integer.fromstr(attr,val);if(typeVal==null)return null;switch(opt){case filterOptions.contains:return function(v){return((v[0]<=typeVal)&&(typeVal<=v[1]));}
case filterOptions.containsNot:return function(v){return((typeVal<v[0])||(v[1]<typeVal));}
case filterOptions.less:return function(v){return v[1]<typeVal;}
case filterOptions.lessEq:return function(v){return v[1]<=typeVal;}
case filterOptions.greater:return function(v){return typeVal<v[0];}
case filterOptions.greaterEq:return function(v){return typeVal<=v[0];}}
return null;};types.integerrange.filters=function(attrs){return[filterOptions.contains,filterOptions.containsNot,filterOptions.less,filterOptions.lessEq,filterOptions.greater,filterOptions.greaterEq];};types.numberlist=inherit(types.def);types.numberlist.tostr=function(attrs,val){if(val==null){val=[];if(attrs.def!=null)val=attrs.def;}
...
return null;}
return[low,high];};types.numberrange.deflimit=function(attrs){var str=(attrs.min||0).toString(attrs.radix||10)+'..';if(attrs.max!=null)str+=attrs.max.toString(attrs.radix||10);return str;};types.numberrange.addlimit=function(attrs,limit,str){if(!limit.ranges)limit.ranges=[];return parseLimit(limit.ranges,str,attrs.min||0,attrs.max||0xffffffff,function(str){var n=string2int(str);if(n==null||n<0)return null;if(!minmax(n,attrs.min,attrs.max))return null;return n;});};types.numberrange.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,trim(val));if(typeVal==null)return null;if(typeVal[0]!=typeVal[1])return null;const number=typeVal[0];switch(opt){case filterOptions.is:return function(v){return(number==v[0])&&(number==v[1]);}
case filterOptions.contains:return function(v){return(number>=v[0])&&(number<=v[1]);}
case filterOptions.containsNot:return function(v){return(number<v[0])||(number>v[1]);}
case filterOptions.less:return function(v){return(number>v[1]);}
case filterOptions.lessEq:return function(v){return(number>=v[1]);}
case filterOptions.greater:return function(v){return(number<v[0]);}
case filterOptions.lessEq:return function(v){return(number<=v[0]);}}
return null;};types.numberrange.filters=function(attrs){return[filterOptions.is,filterOptions.contains,filterOptions.containsNot,filterOptions.less,filterOptions.lessEq,filterOptions.greater,filterOptions.greaterEq];};types.numberrangelist=inherit(types.def);types.numberrangelist.hasValue=function(attrs,val){return val.length>0;};types.numberrangelist.fromstr=function(attrs,str,limit){var v=str.split(',');var a=[];for(var i in v){if(v[i]=='')continue;var e=string2enum(attrs.values,v[i]);if(e!=null){if(!fitsRangeLimit(e,limit))return null;a.push(e);a.push(e);}else{var r=types.numberrange.fromstr(attrs,v[i],limit);if(r==null||r[0]>r[1])return null;a.push(r[0]);a.push(r[1]);}}
...
return str;};types.numberrangelist.deflimit=function(attrs){return types.numberrange.deflimit(attrs);};types.numberrangelist.addlimit=function(attrs,limit,str){return types.numberrange.addlimit(attrs,limit,str);};types.numberrangelist.matcher=function(attr,opt,val){const matchFunc=types.numberrange.matcher(attr,opt,val);if(matchFunc==null)return null;switch(opt){case filterOptions.contains:return function(v){for(var i in v){if(matchFunc(v[i]))return true;}
return false;}
case filterOptions.containsNot:return function(v){for(var i in v){if(!matchFunc(v[i]))return false;}
return true;}}
return null;};types.numberrangelist.filters=function(attrs){return[filterOptions.contains,filterOptions.containsNot];};types.numbertable=inherit(types.def);types.numbertable.view=function(attrs,ro){return new NumberTableView(attrs);};types.table=inherit(types.def);types.table.view=function(attrs,ro){return new TableView(attrs);};types.object=inherit(types.def);types.object.get=function(attrs,obj){var o=obj[attrs.id];if(!o)o=obj[attrs.id]={};if(!o._owner){o._owner=new Listeners();}
...
return str;};types.string.less=function(attrs,s1,s2){return lexicographicalLess(s1,s2);};types.string.deflimit=function(attrs){return'';};types.string.addlimit=function(attrs,limit,str){if(!limit.prefixes)limit.prefixes=[];limit.prefixes.push(str);return true;};types.string.hyperlink=function(attrs,val){return attrs.url?val:null;};types.string.cell=function(col,obj,val){const node=types.def.cell(col,obj,val);if(node&&col.attrs.url){return make.anchor({cont:node.textContent},node.textContent);}
return node;};types.string.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);if(typeVal==null)return null;switch(opt){case filterOptions.is:return function(v){return v==val;}
case filterOptions.isNot:return function(v){return v!=val;}
case filterOptions.contains:return function(v){return v.includes(typeVal);}
case filterOptions.containsNot:return function(v){return!v.includes(typeVal);}}
return null;};types.string.filters=function(attrs){return[filterOptions.is,filterOptions.isNot,filterOptions.contains,filterOptions.containsNot];};types.version=inherit(types.string);types.version.less=function(attrs,v1,v2){return(string2version(v1)??0)<(string2version(v2)??0);};types.secret=inherit(types.string);types.secret.cell=function(col,obj,val){if(col.attrs.cond&&!col.attrs.cond.isTrue(obj))return null;var str='';if(val!=null||!col.attrs.opt)str=ftype(col.attrs).tostr(col.attrs,val);if(hidePasswords){var x='';while(x.length<str.length)x+='*';str=x;}
...
return new SecretView(attrs);};types.secret.matcher=function(attr,opt,val){return null;};types.secret.filters=function(attrs){return[];};types.script=inherit(types.string);types.script.view=function(attrs,ro){if(ro)return new ROTextView(attrs,5);return new TextAreaView(attrs);};types.packet=inherit(types.string);types.packet.view=function(attrs,ro){return new ROPreTextView(attrs);};types.multilinestring=inherit(types.string);types.multilinestring.view=function(attrs,ro){if(attrs.qr)return new ROQRView(attrs);if(ro)return new ROTextView(attrs,attrs.lines);return new TextAreaView(attrs);};types.stringarray=inherit(types.def);types.stringarray.tostr=function(attrs,val){if(val==null)return null;return val.join('\n');};types.stringarray.view=function(attrs,ro){return new ROTextView(attrs,attrs.rows);};types.stringarray.matcher=function(attr,opt,val){switch(opt){case filterOptions.contains:return function(v){return types.stringarray.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.stringarray.tostr(attr,v).includes(val);}}
return null;};types.stringarray.filters=function(attrs){return[filterOptions.contains,filterOptions.containsNot];};types.ipaddr=inherit(types.def);types.ipaddr.put=function(attrs,obj,val){obj[attrs.id||0]=val!=null?val:0;};types.ipaddr.hasValue=function(attrs,val){return val!=0;};types.ipaddr.tostr=function(attrs,val){return ipaddr2string(val||attrs.def||0);};types.ipaddr.fromstr=function(attrs,str){var addr=string2ipaddr(str);if(addr==null)return null;if(attrs.zeroinvalid&&(addr==0))return null;return addr;};types.ipaddr.less=function(attrs,v1,v2){v1=ntohl(v1);v2=ntohl(v2);if(v1<0){if(v2>=0)return false;return v1<v2;}else if(v2<0){return true;}
return v1<v2;};types.ipaddr.matcher=function(attr,opt,val){var typeVal=types.network.fromstr({range:1},val);if(typeVal==null)return null;switch(opt){case filterOptions.in:return function(v){return(v>=typeVal[0])&&(v<=typeVal[1]);}
case filterOptions.inNot:return function(v){return(v<typeVal[0])||(v>typeVal[1]);}}
return null;};types.ipaddr.filters=function(attrs){return[filterOptions.in,filterOptions.inNot];};types.addr=inherit(types.def);types.addr.get=function(attrs,obj){if(attrs.id==null)return obj;return obj[attrs.id];}
types.addr.put=function(attrs,obj,val){if(attrs.id==null){if(val!=null)update(obj,val);}else{obj[attrs.id]=val!=null?val:{};}};types.addr.fromstr=function(attrs,str){types.addr.init(attrs);if(attrs.req&&str==="")return null;const val={};const allow=attrs.allow;let off=-1;const l=str.split(/[/%@&]/);for(let i=0;i<l.length;++i){if(i==0){if(allow.includes('4')&&val.ufeff20==null){const a=string2ipaddr(l[i]);if(a!=null){val.ufeff20=a;continue;}}
if(allow.includes('6')&&val.afeff21==null){const a=string2ip6addr(l[i]);if(a!=null){val.afeff21=a;continue;}}
if(allow.includes('D')&&val.sfeff26==null){val.sfeff26=str;continue;}
...
off+=l[i-1].length+1;switch(str[off]){case'/':if(allow.includes('/')&&val.ufeff25==null){const p=string2int(l[i]);if(p!=null&&p>=0&&p<=128){val.ufeff25=p;break;}}
return null;case'%':if((allow.includes('i')||allow.includes('%'))&&val.ufeff22==null){if(allow.includes('%')){if(nullish(val.ufeff20)&&nullish(val.afeff21)&&nullish(val.afeff21)){return null;}}
const v=string2enum(attrs.ifaces,l[i]);if(v!=null){val.ufeff22=v;break;}}
return null;case'@':if(allow.includes('v')&&val.ufeff23==null){const v=string2enum(attrs.vrfs,l[i]);if(v!=null){val.ufeff23=v;break;}}
return null;case'&':if(allow.includes('R')&&val.sfeff27==null&&parseRd(l[i])){val.sfeff27=l[i];break;}
return null;}}
return val;};types.addr.tostr=function(attrs,val={}){types.addr.init(attrs);const allow=attrs.allow??"";if(allow==="")return"";let str="";if(allow.includes('6')&&val.afeff21!=null){str=ip6addr2string(val.afeff21);}else if(allow.includes('4')&&val.ufeff20!=null){str=ipaddr2string(val.ufeff20);}else if(allow.includes('D')&&val.sfeff26!=null){str=val.sfeff26;}else if(allow.includes('m')&&val.rfeff2f!=null){str=macaddr2string(val.rfeff2f);}
if(allow.includes('/')&&val.ufeff25!=null){str+='/'+val.ufeff25.toString(10);}
if((allow.includes('i')||allow.includes('%'))&&val.ufeff22!=null){const i=enum2string(attrs.ifaces,val.ufeff22);str+='%'+(i!=null?i:val.ufeff22.toString(16));}
if(allow.includes('v')&&val.ufeff23!=null){const v=enum2string(attrs.vrfs,val.ufeff23)
str+='@'+(v!=null?v:"main");}
if(allow.includes('R')&&val.sfeff27!=null){if(str=='')str=val.sfeff27;else str+='&'+val.sfeff27;}
return str;};types.addr.less=function(attrs,s1,s2){types.addr.init(attrs);const allow=attrs.allow??"";if(allow.includes('6')){const a1=s1.afeff21||[];const a2=s2.afeff21||[];if(a1<a2)return true;if(a1>a2)return false;}
if(allow.includes('4')){const a1=s1.ufeff20||0;const a2=s2.ufeff20||0;if(types.ipaddr.less(null,a1,a2))return true;if(types.ipaddr.less(null,a2,a1))return false;}
if(allow.includes('D')){const a1=s1.sfeff26||"";const a2=s2.sfeff26||"";if(a1<a2)return true;if(a1>a2)return false;}
if(allow.includes('/')){const p1=s1.ufeff25||0;const p2=s2.ufeff25||0;if(p1<p2)return true;if(p1>p2)return false;}
if(allow.includes('i')||allow.includes('%')){const i1=enum2string(attrs.ifaces,s1.ufeff22||0xffffffff);const i2=enum2string(attrs.ifaces,s2.ufeff22||0xffffffff);if(i2==null){if(i1!=null)return false;}else if(i1!=null){if(types.string.less(null,i1,i2))return true;if(types.string.less(null,i2,i1))return false;}}
if(allow.includes('v')){const v1=enum2string(attrs.vrfs,s1.ufeff23||0xffffffff);const v2=enum2string(attrs.vrfs,s2.ufeff23||0xffffffff);if(v1<v2)return true;if(v1>v2)return false;}
if(allow.includes('R')){const r1=s1.sfeff27||"";const r2=s2.sfeff27||"";if(r1<r2)return true;if(r1>r2)return false;}
return false;};types.addr.listen=function(attrs,cb){types.addr.init(attrs);if(attrs.ifaces)enm.dynamic.listen(attrs.ifaces,cb);if(attrs.vrfs)enm.dynamic.listen(attrs.vrfs,cb);types.def.listen(attrs,cb);};types.addr.unlisten=function(attrs,cb){if(attrs.ifaces)enm.dynamic.unlisten(attrs.ifaces,cb);if(attrs.vrfs)enm.dynamic.unlisten(attrs.vrfs,cb);types.def.unlisten(attrs,cb);};types.addr.init=function(attrs){if(attrs.ifaces==undefined){attrs.ifaces=null;if(attrs.allow.includes('i')||attrs.allow.includes('%')){attrs.ifaces={type:'dynamic',path:[20,0]};}}
if(attrs.vrfs==undefined){attrs.vrfs=null;if(attrs.allow.includes('v')){attrs.vrfs={type:'dynamic',path:[20,101]};}}};types.addr.hasValue=function(attrs,val){return(Object.keys(val).length==0)?false:true;};types.addr.matcher=function(attr,opt,val){switch(opt){case filterOptions.is:return function(v){return types.addr.tostr(attr,v)==val;}
case filterOptions.contains:return function(v){return types.addr.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.addr.tostr(attr,v).includes(val);}}
return null;};types.addr.filters=function(attrs){return[filterOptions.is,filterOptions.contains,filterOptions.containsNot];};types.ipaddrandport=inherit(types.def);types.ipaddrandport.get=function(attrs,obj){return[obj[attrs.id||0],obj[attrs.portid||1]];};types.ipaddrandport.put=function(attrs,obj,val){if(val==null)val=[0,0];obj[attrs.id||0]=val[0];obj[attrs.portid||1]=val[1];};types.ipaddrandport.remove=function(attrs,obj){delete obj[attrs.id||0];delete obj[attrs.portid||0];unset(obj,attrs.id);unset(obj,attrs.portid);};types.ipaddrandport.tostr=function(attrs,val){var str=ipaddr2string(val[0]);if(val[1])str+=':'+val[1];return str;};types.ipaddrandport.fromstr=function(attrs,str){return null;};types.ipaddrandport.less=function(attrs,v1,v2){var a1=ntohl(v1[0]);var a2=ntohl(v2[0]);if(a1<a2)return true;if(a1>a2)return false;return v1[1]<v2[2];};types.ipaddrandport.matcher=function(attr,opt,val){switch(opt){case filterOptions.is:return function(v){return types.ipaddrandport.tostr(attr,v)==val;}
case filterOptions.contains:return function(v){return types.ipaddrandport.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.ipaddrandport.tostr(attr,v).includes(val);}}
return null;};types.ipaddrandport.filters=function(attrs){return[filterOptions.is,filterOptions.contains,filterOptions.containsNot];};types.netmask=inherit(types.def);types.netmask.fromstr=function(attrs,str){var m=string2int(str);if(m!=null){if(m<0||m>32)return null;return len2netmask(m);}
return string2ipaddr(str);};types.netmask.tostr=function(attrs,val){return netmask2len(val).toString();};types.netmask.less=function(attrs,v1,v2){return ntohl(v1)<ntohl(v2);};types.netmask.matcher=function(attr,opt,val){switch(opt){case filterOptions.is:return function(v){return types.netmask.tostr(attr,v)==val;}}
return null;};types.netmask.filters=function(attrs){return[filterOptions.is];};types.network=inherit(types.def);types.network.get=function(attrs,obj){var addr=obj[attrs.id||0];if(addr==null)return null;var mask=obj[attrs.maskid||1];return[addr,mask];};types.network.put=function(attrs,obj,val){if(val==null)val=[0,0];obj[attrs.id||0]=val[0];obj[attrs.maskid||1]=val[1];};types.network.remove=function(attrs,obj){delete obj[attrs.id||0];delete obj[attrs.maskid||0];unset(obj,attrs.id);unset(obj,attrs.maskid);};types.network.tostr=function(attrs,val){if(val==null)val=[0,0];var addr=val[0]||0;var mask=val[1];if(mask==undefined)mask=attrs.deflen||0;var str=ipaddr2string(addr);if(attrs.range){if(addr==mask)return str;for(var i=31;i>=0;--i){var m=int2num(~len2netmask(i));if(int2num(addr|m)==int2num(mask)&&(addr&m)==0){return str+'/'+i;}}
...
if(a1<a2)return true;if(a1>a2)return false;return ntohl(v1[1])<ntohl(v2[1]);};types.network.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);if(typeVal==null)return null;const rangeCmp=function(v){const r1=ntohl(typeVal[0]);const r2=ntohl(typeVal[1]);const first=ntohl(v[0]);const last=ntohl(v[1]);return first>=r1&&first<=r2&&last<=r2;};const maskCmp=function(v){const first=v[0];const last=first|(~v[1]);return rangeCmp([first,last]);};switch(opt){case filterOptions.is:return function(v){return(v[0]==typeVal[0])&&(v[1]==typeVal[1]);}
case filterOptions.in:return function(v){return attr.range?rangeCmp(v):maskCmp(v);}
case filterOptions.inNot:return function(v){return attr.range?!rangeCmp(v):!maskCmp(v);}}
return null;};types.network.filters=function(attrs){return[filterOptions.is,filterOptions.in,filterOptions.inNot];};types.ip6addr=inherit(types.def);types.ip6addr.get=function(attrs,obj){var addr=obj[attrs.id||0];if(addr==null)return null;if(attrs.ifaceid)return[addr,obj[attrs.ifaceid]||0];return addr;};types.ip6addr.put=function(attrs,obj,val){if(attrs.ifaceid){if(val==null)val=[fillarray(0,16),0];obj[attrs.id]=val[0];obj[attrs.ifaceid]=val[1];}else{obj[attrs.id||0]=val!=null?val:fillarray(0,16);}};types.ip6addr.remove=function(attrs,obj){delete obj[attrs.id||0];if(attrs.ifaceid)delete obj[attrs.ifaceid];unset(obj,attrs.id);unset(obj,attrs.ifaceid);};types.ip6addr.tostr=function(attrs,val){if(val==null)return attrs.allowipv4?'0.0.0.0':'::';if(attrs.ifaceid){if(!attrs.ifaces){attrs.ifaces={type:'dynamic',path:[20,0]};}
...
types.def.unlisten(attrs,cb);};types.ip6addr.matcher=function(attr,opt,val){var typeVal=types.network6.fromstr(attr,val);if(typeVal==null)return null;const addrVal=typeVal[0];const bitLen=typeVal[1];const mask=function(v,len){const fullBits=len/8;var masked=fillarray(0,16);for(var i=0;i<fullBits;i++){masked[i]=v[i];}
masked[fullBits]=v[fullBits]&~((1<<(8-(len&7)))-1);return masked;}
switch(opt){case filterOptions.in:return function(v){const cmpVal=mask(v,bitLen);for(const i in addrVal){if(addrVal[i]!=cmpVal[i])return false;}
return true;}
case filterOptions.inNot:return function(v){const cmpVal=mask(v,bitLen);for(const i in addrVal){if(addrVal[i]!=cmpVal[i])return true;}
return false;}}
return null;};types.ip6addr.filters=function(attrs){return[filterOptions.in,filterOptions.inNot];};types.network6=inherit(types.network);types.network6.put=function(attrs,obj,val){if(val==null)val=[fillarray(0,16),0];obj[attrs.id||0]=val[0];obj[attrs.maskid||1]=val[1];};types.network6.tostr=function(attrs,val){if(val==null)return'::/'+(attrs.deflen||0);var addr=val[0]!=null?ip6addr2string(val[0]):"::";if(val[1]==128)return addr;return addr+'/'+(val[1]||0);};types.network6.fromstr=function(attrs,str){var v=str.split('/');var addr=string2ip6addr(v[0]);if(addr==null)return null;var len=128;if(v.length==2){len=string2int(v[1]);if(len==null||len<0||len>128)return null;}
...
return null;}
return[addr,len];};types.network6.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);if(typeVal==null)return null;const mask=function(v,len){const fullBits=len/8;var masked=fillarray(0,16);for(var i=0;i<fullBits;i++){masked[i]=v[i];}
masked[fullBits]=v[fullBits]&~((1<<(8-(len&7)))-1);return masked;}
switch(opt){case filterOptions.in:return function(v){if(v[1]<typeVal[1])return false;const cmpVal=mask(v[0],typeVal[1]);for(const i in typeVal[0]){if(typeVal[0][i]!=cmpVal[i])return false;}
return true;}
case filterOptions.inNot:return function(v){if(v[1]<typeVal[1])return true;const cmpVal=mask(v[0],typeVal[1]);for(const i in typeVal[0]){if(typeVal[0][i]!=cmpVal[i])return true;}
return false;}}
return null;};types.network6.filters=function(attrs){return[filterOptions.in,filterOptions.inNot];};types.macaddr=inherit(types.def);types.macaddr.put=function(attrs,obj,val){if(val==null)val=[0,0,0,0,0,0];obj[attrs.id||0]=val;};types.macaddr.hasValue=function(attrs,val){if(val==null)return false;for(var i=0;i<6;++i){if(val[i])return true;}
...
return false;};types.macaddr.deflimit=function(attrs){return'';};types.macaddr.addlimit=function(attrs,limit,str){if(!limit.prefixes)limit.prefixes=[];limit.prefixes.push(str);return true;};types.macaddr.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);if(typeVal==null)return null;switch(opt){case filterOptions.in:return function(v){if(v.length!=6)return false;for(var i in typeVal){if(v[i]!=typeVal[i])return false;}
return true;}
case filterOptions.inNot:return function(v){if(v.length!=6)return false;for(var i in typeVal){if(v[i]!=typeVal[i])return true;}
return false;}}
return null;};types.macaddr.filters=function(attrs){return[filterOptions.in,filterOptions.inNot];};types.macnetwork=inherit(types.def);types.macnetwork.get=function(attrs,obj){var addr=obj[attrs.id||0];var mask=obj[attrs.maskid||1];if(addr==null||mask==null)return null;return[addr,mask];}
...
return[addr,mask];};types.macnetwork.deflimit=function(attrs){return types.macaddr.deflimit(attrs);};types.macnetwork.addlimit=function(attrs,limit,str){return types.macaddr.addlimit(attrs,limit,str);};types.macnetwork.matcher=function(attr,opt,val){switch(opt){case filterOptions.is:return function(v){return types.macnetwork.tostr(attr,v)==val;}
case filterOptions.contains:return function(v){return types.macnetwork.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.macnetwork.tostr(attr,v).includes(val);}}
return null;};types.macnetwork.filters=function(attrs){return[filterOptions.is,filterOptions.contains,filterOptions.containsNot];};types.date=inherit(types.def);types.date.tostr=function(attrs,val){return dateAndTime2string(getDate(val),getTime(val),false);};types.date.fromstr=function(obj,attrs,str){return null;};types.dateandtime=inherit(types.def);types.dateandtime.get=function(attrs,obj){if(attrs.timeid){if(obj[attrs.id]==null)return null;return[obj[attrs.id],obj[attrs.timeid]];}
...
if(!limit.prefixes)limit.prefixes=[];limit.prefixes.push(str);return true;};types.enm.deferred=function(attrs){return attrs.values?.type==="dynamic";};types.enm.matcher=function(attr,opt,val){var typeVal=this.fromstr(attr,val);switch(opt){case filterOptions.is:return function(v){if(typeVal==null)return false;return v==typeVal;}
case filterOptions.isNot:return function(v){if(typeVal==null)return true;return v!=typeVal;}
case filterOptions.contains:return function(v){return types.enm.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.enm.tostr(attr,v).includes(val);}}
return null;};types.enm.filters=function(attrs){return[filterOptions.is,filterOptions.isNot,filterOptions.contains,filterOptions.containsNot];};types.set=inherit(types.def);types.set.get=function(attrs,obj){var val=obj[attrs.id||0];if(val==null)return null;if(attrs.id&&attrs.id.charAt(0)=='U'){if(val.length==0)return null;return[val[0],(val[1]!=null?val[1]:~val[0])];}
...
if(limit.invert&&!invert)return;if(!limit.invert&&invert){limit.prefixes=null;limit.invert=true;}
if(!limit.prefixes)limit.prefixes=[];limit.prefixes.push(str);return true;};types.set.matcher=function(attr,opt,val){switch(opt){case filterOptions.is:return function(v){return types.set.tostr(attr,v)==val;}
case filterOptions.contains:return function(v){return types.set.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.set.tostr(attr,v).includes(val);}}
return null;};types.set.filters=function(attrs){return[filterOptions.is,filterOptions.contains,filterOptions.containsNot];};types.hyperlink=inherit(types.def);types.hyperlink.get=function(attrs,obj){return obj[attrs.id||0];};types.hyperlink.put=function(attrs,obj,val){};types.hyperlink.hasValue=function(attrs,val){return val!=null&&val!=0xffffffff;};types.hyperlink.tostr=function(attrs,val){if(!attrs.enm)attrs.enm={type:'dynamic',path:attrs.path};if(val==null)return'unknown';var str=enum2string(attrs.enm,val);return str?str:'*'+val.toString(16);};types.hyperlink.hyperlink=function(attrs,val){if(val==null)return null;var a=mapAttrs[attrs.path];if(a==null)return null;return getPath(a)+'.'+val;};types.hyperlink.view=function(attrs,ro){return types.def.view(attrs,true);};types.hyperlink.listen=function(attrs,cb){if(!attrs.enm)attrs.enm={type:'dynamic',path:attrs.path};enm.dynamic.listen(attrs.enm,cb);types.def.listen(attrs,cb);};types.hyperlink.unlisten=function(attrs,cb){enm.dynamic.unlisten(attrs.enm,cb);types.def.unlisten(attrs,cb);};types.hyperlink.matcher=function(attr,opt,val){switch(opt){case filterOptions.contains:return function(v){return types.hyperlink.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.hyperlink.tostr(attr,v).includes(val);}}
return null;};types.hyperlink.filters=function(attrs){return[filterOptions.contains,filterOptions.containsNot];};types.raw=inherit(types.def);types.raw.tostr=function(attrs,val){let str='';try{for(let i=0;i<val.length;i++){str+=val[i].toString(16).padStart(2,'0');}}catch(e){}
...
return val;};types.raw.matcher=function(attr,opt,val){switch(opt){case filterOptions.contains:return function(v){return types.raw.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.raw.tostr(attr,v).includes(val);}}
return null;};types.raw.filters=function(attrs){return[filterOptions.contains,filterOptions.containsNot];};types.lookup=inherit(types.def);types.lookup.updateCache=function(attrs){var map=getObjectMap(attrs.path);if(map==null)return;var res={}
...
types.lookup.listen=function(attrs,cb,ctrl,obj){if(!attrs.cb){types.lookup.updateCache(attrs);var map=getObjectMap(attrs.path);attrs.cb=function(){types.lookup.updateCache(attrs);attrs.lstns.notify();};attrs.lstns=new Listeners();if(map){attrs._dst=getAttr(map.getType(),attrs.dst);attrs._val=getAttr(map.getType(),attrs.val);ftype(attrs._dst).listen(attrs._dst,attrs.cb);ftype(attrs._val).listen(attrs._val,attrs.cb);map.listen(attrs.cb,true);}}
...
return{limits:limits};};types.tuple.matcher=function(attr,opt,val){switch(opt){case filterOptions.contains:return function(v){return types.tuple.tostr(attr,v).includes(val);}
case filterOptions.containsNot:return function(v){return!types.tuple.tostr(attr,v).includes(val);}}
return null;};types.tuple.filters=function(attrs){return[filterOptions.contains,filterOptions.containsNot];};types.union=inherit(types.def);types.union.get=function(attrs,obj){if(attrs.id){let i=obj[attrs.id]||attrs.def||0;if(attrs.mapping!=null)i=attrs.mapping[i]||0;if(i>=attrs.c.length)return[0,null];let d=ftype(attrs.c[i]).get(attrs.c[i],obj,true);return[i,d];}else if(attrs.single){for(let i in attrs.c){let d=ftype(attrs.c[i]).get(attrs.c[i],obj,true);if(d!=null){return[Number(i),d];}}
...
return v1.length<v2.length;};types.multi.view=function(attrs,ro){return new MultiView(attrs,ro,function(){return ftype(attrs.c[0]).view(attrs.c[0],ro);});};types.multi.deflimit=function(attrs){return ftype(attrs.c[0]).deflimit(attrs.c[0]);};types.multi.addlimit=function(attrs,limit,str){return ftype(attrs.c[0]).addlimit(attrs.c[0],limit,str);};types.multi.matcher=function(attr,opt,val){const matchFunc=ftype(attr.c[0]).matcher(attr.c[0],opt,val);if(matchFunc==null)return null;switch(opt){case filterOptions.contains:case filterOptions.in:case filterOptions.is:return function(v){for(var i in v){if(matchFunc(v[i]))return true;}
return false;}
case filterOptions.containsNot:case filterOptions.inNot:case filterOptions.isNot:return function(v){for(var i in v){if(!matchFunc(v[i]))return false;}
return true;}}
return null;};types.multi.filters=function(attrs){return ftype(attrs.c[0]).filters(attrs.c[0]).filter((el)=>{switch(el){case filterOptions.contains:case filterOptions.in:case filterOptions.is:case filterOptions.containsNot:case filterOptions.inNot:case filterOptions.isNot:return true;default:return false;}});};types.multinumber=inherit(types.multi);types.multinumber.get=function(attrs,obj){if(attrs.optid!=null&&obj[attrs.optid]==false)return null;var val=obj[attrs.id];if(val==null)return val;var a=new Array(val.length);for(var i in val)a[i]={0:val[i]};return a;};types.multinumber.put=function(attrs,obj,val){var a=[];for(var i in val){if(val[i][0]!=null)a.push(val[i][0]);}
obj[attrs.id]=a;if(attrs.optid!=null)obj[attrs.optid]=a.length>0?1:0;};types.multinumber.matcher=function(attr,opt,val){const matchFunc=ftype(attr.c[0]).matcher(attr.c[0],opt,val);if(matchFunc==null)return null;switch(opt){case filterOptions.contains:case filterOptions.in:return function(v){for(var i in v){if(matchFunc(v[i][0]))return true;}
return false;}
case filterOptions.containsNot:case filterOptions.inNot:return function(v){for(var i in v){if(!matchFunc(v[i][0]))return false;}
return true;}}
return null;};types.multinumber.filters=function(attrs){return ftype(attrs.c[0]).filters(attrs.c[0]).filter((el)=>{switch(el){case filterOptions.contains:case filterOptions.in:case filterOptions.containsNot:case filterOptions.inNot:return true;default:return false;}});};types.multinumberrange=inherit(types.multi);types.multinumberrange.get=function(attrs,obj){if(attrs.optid!=null&&obj[attrs.optid]==false)return null;var val=obj[attrs.id];if(val==null)return val;if(attrs.id2!=null){var val2=obj[attrs.id2];if(val2==null)val2=[];let a=new Array(val.length);for(let i=0;i<val.length;++i){a[i]={0:val[i],1:val2[i]};}
return a;}
...
return null;};types.deck.columns=function(attrs,table,type){const colArr=[];attrs.panes?.forEach(pane=>{pane.c?.forEach(cont=>{if(cont.width){colArr.push({name:cont.name,width:cont.width,deckPane:pane,deckCont:cont});}});});return colArr;};types.deck.cell=function(col,obj,val,cell){if(col.deckCont&&col.deckPane){const onAttr=getAttr(obj._type,col.attrs.selon);const selonVal=ftype(onAttr).get(onAttr,obj);if(col.deckPane.vals.includes(selonVal)){val=ftype(col.deckCont).get(col.deckCont,obj);var result=ftype(col.deckCont).cell({attrs:col.deckCont},obj,val,cell);return result;}}
...
++i;}
return val;};types.about.column=function(attrs,table,type){table.addAboutInfo(attrs);};types.about.view=function(attrs,ro){return new AboutView(attrs);};types.columnalias=inherit(types.def);types.columnalias.alias=true;types.columnalias.column=function(attrs,table,type){if(!attrs.alias)attrs.alias=getAttr(type,attrs.name);if(attrs.alias){var c=ftype(attrs.alias).column(attrs.alias,table,type);if(c)return{name:c.name,width:attrs.width};}
return null;};types.columnalias.cell=function(col,obj,val,cell){if(col.attrs.alias){val=ftype(col.attrs.alias).get(col.attrs.alias,obj);return ftype(col.attrs.alias).cell({attrs:col.attrs.alias},obj,val,cell);}
return null;};types.columnalias.listen=function(attrs,cb){if(attrs.alias)ftype(attrs.alias).listen(attrs.alias,cb);};types.columnalias.unlisten=function(attrs,cb){if(attrs.alias)ftype(attrs.alias).unlisten(attrs.alias,cb);};types.columnalias.view=function(){return null;};types.columnalias.lookup=function(attrs,owner,name,nr){return null;};types.columnalias.get=function(attrs,obj){return types[attrs.alias.type].get(attrs.alias,obj);};types.columnalias.matcher=function(attr,opt,val){return types[attr.alias.type].matcher(attr.alias,opt,val);};types.columnalias.filters=function(attrs){return types[attrs.alias.type].filters(attrs.alias);};types.alias=inherit(types.def);types.alias.alias=true;types.alias.view=function(attrs,ro){return new AliasView(attrs,ro);};types.alias.lookup=function(attrs,owner,name,nr){return null;};types.graph=inherit(types.def);types.graph.get=function(attrs,obj){if(!attrs.model)return null;return ftype(attrs.model).get(attrs.model,obj);};types.graph.put=function(attrs,obj,val){if(attrs.model){ftype(attrs.model).put(attrs.model,obj,val);}};types.graph.view=function(attrs){return new GraphView(attrs);};types.graph.column=function(attrs,table,type){if(attrs.model&&attrs.width)return types.def.column(attrs,table,type);return null;};types.graph.cell=function(col,obj,val,canvas){if(val==null)return null;const g=createGraphic(canvas,200,16,80,16);if(g==null)return null;g.canvas.style.display='block';g.strokeWidth=3;drawBars(g,val,col.attrs.offset,col.attrs.max,col.attrs.colors);return g.canvas;};types.graphbox=inherit(types.def);types.graphbox.get=function(attrs,obj){var r=[];for(var g in attrs.graphs){r.push(ftype(attrs.graphs[g]).get(attrs.graphs[g],obj));}
...
return null;};types.file=inherit(types.def);types.file.column=function(attrs,table,type){if(!(sysres.policy&(1<<4)))return null;if(sysres.policy&(1<<7)){const view=new FileUploadView(attrs);const input=view.create();input.id="uploader-field";const str=getProp(getProp(skin,guiStrings.commonNames),guiStrings.upload).name??guiStrings.upload;const btn=current.addActionButton(str);btn.onclick=()=>input.click();}
...
return true;}
if(obj&&!downloadable(obj))return;const str=getProp(getProp(skin,guiStrings.commonNames),guiStrings.download).name??guiStrings.download;const b=current.addActionButton(str,cfg,obj?._owner?.toString(obj));const downloadFile=obj=>{const path=obj._owner.toString(obj);const link=make.anchor({},"/jsproxy/?"+session.encryptURI(path));link.download=path;link.click();}
b.onclick=e=>{if(obj)downloadFile(obj);else[...ctrl.selected].map(sel=>sel.obj()).filter(downloadable).forEach(downloadFile);e.preventDefault();}
if(!b.href){b.disabled=true;ctrl.selectLstns?.listen(()=>{b.disabled=![...ctrl.selected].map(sel=>sel.obj()).filter(downloadable).length;});}else{b.onkeydown=e=>{if(e.key!=="Enter")return;b.click();e.preventDefault();}}}
...
b=current.addActionButton(attrs.name,cfg);b.onclick=function(e){if(!attrs.confirm||confirm(attrs.confirm)){var doit=new Doit(attrs,type.path);if(obj)doit.setID(obj);doit.doit();}};}else{if(!attrs.name)return;if(!(ctrl instanceof Table))return;b=current.addActionButton(attrs.name,cfg);b.disabled=true;get(`${b.id}-opt`).disabled=true;b.dataset.perRow=true;ctrl.selectLstns?.listen(()=>{b.disabled=!ctrl.selected.size;get(`${b.id}-opt`).disabled=b.disabled;});b.onclick=e=>{if(!attrs.confirm||confirm(attrs.confirm)){ctrl.selected.forEach(row=>{const doit=new Doit(attrs,type.path);doit.setID(row.obj());doit.doit();})}}}
...
pred.supportmsgid={};pred.supportmsgid.isTrue=function(attrs,val){return true;}
pred.message={};pred.message.isTrue=function(attrs,val){if(val==null)val='';return val==attrs.value;}
...
return ret;}
function postAliveCheck(){post(keepAliveMsg,rep=>{post_notification_request();},err=>{disconnect(err);});}
function post_notification_request(){var start=new Date();post({},receive,function(err){var duration=(new Date()).getTime()-start.getTime();if(err.request.status!=0||duration<1000){postAliveCheck();return;}
...
function getProp(parent,prop){var val=parent[prop];if(val==null)val={};return val;}
...
function getObjectName(obj){const i=obj._owner.getFullName(obj);const name=i.cfg?.[i.propname]??i.name;if(i.postfix)return`${name} ${i.postfix}`;return name;}
class ContainerInterface{setObject(obj,cb){}
getType(){return{}}
getName(obj){return""}
listen(cb,full){}
unlisten(cb,full){}
toString(obj){return""}
getFullName(obj){return{}}}
class ObjectHolder extends ContainerInterface{#attrs;#lstns;#obj;constructor(attrs){super();this.#attrs=attrs;this.#lstns=new Listeners();this.#obj={_type:attrs,_owner:this,_empty:true,};if(attrs.getcmd==null)this.#obj._empty=false;}
getNameType(obj){return null;}
getName(obj){return getObjectName(obj);}
getFullName(obj){return{name:this.#attrs.title,cfg:getSysProps(this.#attrs),propname:'title'};}
getPath(){return getPath(this.#attrs);}
getObject(){return this.#obj;}
setObject(obj,cb){if(this.#obj==obj){const req={};update(req,obj._exportObj||obj);delete obj._exportObj;req.Uff0001=this.#attrs.path;req.uff0007=this.#attrs.setcmd||0xfe000e;copyIdTags(req,obj);delete obj.Uff0014;post(req,rep=>{if(isError(rep)){showError(`Couldn't change ${obj._owner.getName(obj)}`,rep);this.fetch();}else{notifications.info("Saved");}
if(cb)cb(!isError(rep));});this.#lstns.notify(obj);}}
autostart(obj){return false;}
listen(cb){if(this.#lstns.listen(cb)){if(this.#attrs.autorefresh){this.autorefresh=this.#attrs.autorefresh;if(!this.timer)this.fetch();}else{this.notifyLstn=(msg)=>{const objs=msg.Mfe0002;if(!objs){this.fetch();}else if(objs[0]&&(objs[0]._id==null)){this.#obj._empty=false;updateOnNotification(this.#obj,objs[0]);this.#lstns.notify(this.#obj);}};if(!(this.#attrs.getcmd==null&&this.#attrs.setcmd!=null)){subscribe(this.#attrs.path,this.notifyLstn);this.fetch();}}}}
unlisten(cb){if(this.#lstns.unlisten(cb)){if(this.timer){cancelTimer(this.timer);this.timer=null;}
this.autorefresh=0;if(this.notifyLstn){unsubscribe(this.#attrs.path,this.notifyLstn);this.notifyLstn=null;}}}
fetch(){const attrs=this.#attrs;if(attrs.getcmd==null&&attrs.setcmd!=null)return;const req={};req.Uff0001=attrs.path;req.uff0007=attrs.getcmd||0xfe000d;req.ufe000c=0x10000005;req.ufe000c|=attrs.refetchonopen??0;req.ufe000c|=attrs.refreshfilter??0;const onreply=rep=>{if(isError(rep))return;update(this.#obj,rep);this.#obj._empty=false;this.#lstns.notify(this.#obj);if(this.autorefresh&&!this.timer){this.timer=timer(()=>{this.timer=null;this.fetch();},this.autorefresh);}};post(req,onreply);}}
class ObjectMap{#attrs;#lstns;#autorefresh;#map;#acquired;#objCount;#size;#fresh;#timer;#currId;#copySource;#first;#last
constructor(attrs,lstns=new Listeners()){this.#attrs=attrs;this.#lstns=lstns;this.#autorefresh=0;this.#map={};this.#acquired={};this.#objCount=null;this.#size=0;this.#fresh=false;this.#timer=null;this.#copySource=null;this.#first=null;this.#last=null;}
getType(){return this.#attrs;}
setID(obj){this.#currId={_id:obj._id,ufe0001:obj.ufe0001,mfe0026:obj.mfe0026};}
getCount(){return this.#objCount;}
getSize(){return this.#size;}
getNameType(obj){const attrs=obj._type;if(!attrs.namevalAttr){if(typeof attrs.nameval==="string"){attrs.namevalAttr=getAttr(attrs,attrs.nameval);}else{attrs.namevalAttr=this.#attrs.nameval;}}
return attrs.namevalAttr;}
toString(obj){return toString(this.getNameType(obj),obj);}
getName(obj){return getObjectName(obj);}
getFullName(obj){const i={name:this.#attrs.name,cfg:getSysProps(this.#attrs),propname:"title"};if(obj._type.nameval){if("_id"in obj){i.propname="name";i.postfix=`<${this.toString(obj)}>`;}else{i.name=`New ${i.cfg?.name ?? i.name}`;i.propname="newname";}}
return i;}
getPath(obj){const path=getPath(this.#attrs);if(obj){if(!obj._type.nameval&&!obj._type.name)return null;return`${path}.${obj._id ?? "new"}`;}
return path;}
getObject(id){if(id==null)return this.newObject();return this.#map[id];}
newObject(typeid){const type=typeid?this.getSubtype(this.#attrs,typeid)||this.#attrs:this.#attrs;const obj={_type:type,_owner:this,};const subtypeObj={};this.#setSubtype(type,subtypeObj);update(obj,subtypeObj);obj._subtype=new Map(Object.entries(subtypeObj));if(type.prefix)this.#setUniqueName(obj);return obj;}
acquireObject(id){{const obj=this.#acquired[id];if(obj){++obj._refcnt;return obj;}}
let obj=this.getObject(id);if(!obj){obj=this.newObject();obj._id=id;}
obj._refcnt=1;this.#acquired[id]=obj;return obj;}#setUniqueName(obj){const nums={};const type=obj._type;for(const i in this.#map){const name=this.toString(this.#map[i]);if(name.substr(0,type.prefix.length)==type.prefix){const n=string2int(name.substr(type.prefix.length));if(n!=null)nums[n]=true;}}
let i=1;while(nums[i])++i;this.toString(obj);const a=type.namevalAttr;ftype(a).put(a,obj,type.prefix+i);}
setObject(obj,cb){const newObj=obj._id==null;const req={};update(req,obj._exportObj||obj);obj._subtype?.forEach((value,key)=>req[key]=value);delete obj._exportObj;req.Uff0001=this.#attrs.path;req.uff0007=!newObj?0xfe0003:0xfe0005;copyIdTags(req,obj);delete obj.Uff0014;post(req,rep=>{if(isError(rep)){const type=`Couldn't ${newObj ? "add" : "change"} ${this.getName(obj)}`;const str=rep.sff0009??getErrorDescription(rep.uff0008);displayErrorAtButtons(`${str} (${rep.uff0008 & 0xfff})`);notifications.error(type);if(cb)cb(false);if(!newObj)this.fetch(obj);return;}else{notifications.info("Saved");}
if(newObj){++this.#size;if(this.#objCount!=null)++this.#objCount;copyIdTags(obj,rep);assignId(obj);this.#map[obj._id]=obj;if(this.#attrs.ordered)this.#linkBefore(obj,null);}
if(this.#attrs.refetchonchange)this.fetch(obj);if(cb)cb(true);this.#lstns.notify(obj);});}
removeObject(obj,cb){const req={};copyIdTags(req,obj);req.Uff0001=this.#attrs.path;req.uff0007=0xfe0006;post(req,rep=>{if(isError(rep)){showError(`Couldn't remove ${this.getName(obj)}`,rep);if(cb)cb(false);return;}else{notifications.info("Removed");}
this.remove(obj);if(cb)cb(true);});}
moveObjectAfter(obj,after){const next=after!=null?after._next:this.#first;const req={Uff0001:this.#attrs.path,uff0007:0xfe0007,ufe0005:next!=null?next.ufe0001:0xffffffff,}
copyIdTags(req,obj);post(req,rep=>{if(isError(rep)){showError(`Couldn't move ${this.getName(obj)}`,rep);return;}else{notifications.info("moved");}
this.#unlink(obj);this.#linkBefore(obj,next);this.#lstns.notify(obj);});}
remove(obj){if(this.#attrs.ordered){this.#unlink(obj);}
obj.ufe0013=1;delete this.#map[obj._id];--this.#size;if(this.#objCount!=null)--this.#objCount;this.#lstns.notify(obj);}
getSubtypes(){if(!this.#attrs.generic)return null;const types=[];if(this.getSubtypesFor(this.#attrs,types))return types;return null;}
getSubtypesFor(gen,types){let subtypes=0;for(const i in gen.subtypes){const type=gen.subtypes[i];if(type.generic)subtypes+=this.getSubtypesFor(type,types);if(type.nonaddable||type.ro||(type.pred&&!isTrue(type.pred)))continue;types.push(type.title);++subtypes;}
for(const i in gen.gensubtypes){subtypes+=this.getSubtypesFor(gen.gensubtypes[i],types);}
return subtypes;}
getSubtype(gen,id){for(const i in gen.subtypes){if(gen.subtypes[i].ntitle==id)return gen.subtypes[i];if(gen.subtypes[i].generic){const st=this.getSubtype(gen.subtypes[i],id);if(st)return st;}}
for(const i in gen.gensubtypes){const st=this.getSubtype(gen.gensubtypes[i],id);if(st)return st;}
return null;}#setSubtype(attrs,obj,typeid){if(attrs.typeon&&typeid!=null){ftype(attrs.typeon).put(attrs.typeon,obj,typeid);}
if(attrs.inherit){let t=attrs.typevalue;if(t==null||t==0xffffffff)t=typeid;this.#setSubtype(attrs.inherit,obj,t);}}
setDefaultConf(obj){if(this.defconf){updateOnNotification(obj,this.defconf);return;}
const req={Uff0001:this.#attrs.path,uff0007:this.#attrs.getallcmd||0xfe0004,ufe0018:0,ufe000c:0x20000000,};post(req,rep=>{if(isError(rep))return;this.defconf=rep.mfe001d;if(this.defconf){delete this.defconf.bfe0008;const nameTag=obj._type?.namevalAttr?.id;if(nameTag)delete this.defconf[nameTag];updateOnNotification(obj,this.defconf);this.#lstns.notify(obj);}else{this.defconf={};}});}
setCopyObj(obj){this.#copySource=obj;}
copyConf(obj){if(this.#copySource){const nameType=obj._type?.namevalAttr?.type;const nameTag=(nameType==="string")?obj._type?.namevalAttr?.id:null;obj.Sfe001c??=[];for(const i in this.#copySource){if((i===nameTag)||isRoTag(i,obj._owner)||isFlag(i,obj._owner)||(i.charAt(0)==='_')){continue;}
obj[i]=structuredClone(this.#copySource[i]);}
this.copySource=null;delete obj.ufe0001;delete obj.mfe0026;delete obj.bfe0007;delete obj.bfe0008;delete obj.bfe000d;}else{return this.setDefaultConf(obj);}}
fetch(obj){const req={Uff0001:this.#attrs.path,uff0007:0xfe0002,};copyIdTags(req,obj);assignId(obj);post(req,rep=>{if(isError(rep))return;removeSysFields(rep);update(obj,rep);copyIdTags(obj,req);assignId(obj);this.#lstns.notify(obj);});}
foreach(cb){if(this.#attrs.ordered){let obj=this.#last;while(obj!=null){cb(obj);obj=obj._prev;}}else{for(const i in this.#map)cb(this.#map[i]);}}
getFirst(){return this.#first;}
getNext(obj){return obj._next;}
getNextID(obj){return obj._next?.ufe0001??null;}
autostart(obj){this.setObject(obj);return true;}
listen(cb,full){if(full){if(++this.#autorefresh==1&&this.#attrs.autorefresh){this.getall();}}
if(!this.#lstns.listen(cb,full))return;if(this.#attrs.path==null)return;this.notifyLstn=(msg)=>{this.#listenCb(msg)};subscribe(this.#attrs.path,this.notifyLstn);this.getall();}#listenCb(msg){if(msg.ufe0019!=null)this.#objCount=msg.ufe0019;const objs=msg.Mfe0002;if(!objs){this.getall();return;}
for(const i in objs){let obj=objs[i];const id=obj._id;if(id==null)continue;const nextid=obj.ufe0005;if(!obj.ufe0013){const oldObj=this.#map[id];if(!oldObj){const aobj=this.#acquired[id];if(aobj){update(aobj,obj);obj=aobj;}
obj._type=this.#getObjType(obj);obj._owner=this;++this.#size;this.#map[id]=obj;}else{updateOnNotification(oldObj,obj);obj=oldObj;}
if(this.#attrs.ordered){const next=this.#map[nextid];if(obj._next!=next||!oldObj){if(oldObj)this.#unlink(obj);this.#linkBefore(obj,next);}}}else{obj=this.#map[id];if(!obj)continue;obj.ufe0013=1;--this.#size;if(this.#attrs.ordered)this.#unlink(obj);delete this.#map[id];}
this.#lstns.notify(obj,Number(i)<objs.length-1);}}
unlisten(cb,full){if(this.#lstns.unlisten(cb,full)){if(this.#attrs.path!=null){unsubscribe(this.#attrs.path,this.notifyLstn);this.notifyLstn=null;}}
if(full)--this.#autorefresh;}
blockGetall(block){this.block=block;}
getall(cb,req){const attrs=this.#attrs;if(this.getallinprogress||this.block)return;req??={};req.Uff0001=attrs.path;req.uff0007=attrs.getallcmd||0xfe0004;req.ufe000c=0x10000005;req.ufe000c|=attrs.refetchonopen??0;req.ufe000c|=attrs.refreshfilter??0;req.ufe0018=attrs.maxobjs;copyIdTags(req,this.#currId);const oldids={};for(const i in this.#map){oldids[i]=true;}
this.getallinprogress=true;let prev=null;const onreply=rep=>{if(rep.ufe0019!=null)this.#objCount=rep.ufe0019;const objs=rep.Mfe0002??[];for(let obj of objs){const oldObj=this.#map[obj._id];if(oldObj){update(oldObj,obj);obj=oldObj;}else{const aobj=this.#acquired[obj._id];if(aobj){update(aobj,obj);obj=aobj;}
++this.#size;this.#map[obj._id]=obj;}
if(attrs.ordered){if(obj._prev!=prev||!oldObj){if(oldObj)this.#unlink(obj);this.#linkAfter(obj,prev);}
prev=obj;}
obj._type=this.#getObjType(obj);obj._owner=this;delete oldids[obj._id];}
if(cb&&!cb(rep)){this.getallinprogress=false;}else if(isError(rep)){this.getallinprogress=false;if(rep.uff0008===0xfe0004){for(let i in this.#map){const obj=this.#map[i];--this.#size;obj.ufe0013=1;if(attrs.ordered)this.#unlink(obj);delete this.#map[i];this.#lstns.notify(obj);}
this.getall(cb);return;}else if(rep.uff0008==16646158){showError(`Failed to get all ${this.#attrs.maxobjsmsg ?? ""}`,rep);}}else if((rep.ufe0003!=null||rep.mfe0015)&&!this.block){if(rep.ufe0003!=null)req.ufe0003=rep.ufe0003;if(rep.mfe0015!=null)req.mfe0015=rep.mfe0015;post(req,onreply);}else{for(const id in oldids){const obj=this.#map[id];if(!obj)continue;--this.#size;obj.ufe0013=1;if(attrs.ordered)this.#unlink(obj);delete this.#map[id];this.#lstns.notify(obj);}
this.getallinprogress=false;this.#fresh=true;if(!this.block&&!this.#timer&&this.#autorefresh>=1&&attrs.autorefresh){this.#timer=timer(()=>{this.#timer=null;if(this.#autorefresh>=1)this.getall(cb);},attrs.autorefresh);}}
for(const i in objs){const obj=this.#map[objs[i]._id];this.#lstns.notify(obj,this.getallinprogress||Number(i)<objs.length-1);}};post(req,onreply);}#getObjType(obj){return this.#getObjSpecType(obj,this.#attrs)||this.#attrs;}#getObjSpecType(obj,attrs){if(!attrs.generic)return null;let t=attrs.subtypes[obj[attrs.typeon.id]];if(t)return this.#getObjSpecType(obj,t)||t;for(var i in attrs.gensubtypes){t=this.#getObjSpecType(obj,attrs.gensubtypes[i]);if(t)return t;}
return null;}#renumurate(obj,skipNotify){if(!obj)return;let num=obj._prev?obj._prev._num+1:0;do{if(obj._num==num)break;obj._num=num;if(skipNotify>0){--skipNotify;}else{this.#lstns.notify(obj);}
obj=obj._next;++num;}while(obj);}#unlink(obj){if(obj._next){obj._next._prev=obj._prev;this.#renumurate(obj._next,0);}else{this.#last=obj._prev;if(this.#last)this.#last._next=null;}
if(obj._prev){obj._prev._next=obj._next;}else{this.#first=obj._next;if(this.#first)this.#first._prev=null;}}#linkBefore(obj,next){if(next){obj._next=next;obj._prev=next._prev;if(next._prev){next._prev._next=obj;}else{this.#first=obj;}
next._prev=obj;}else{obj._next=null;obj._prev=this.#last;if(obj._prev){obj._prev._next=obj;}else{this.#first=obj;}
this.#last=obj;}
this.#renumurate(obj,1);}#linkAfter(obj,prev){if(prev){obj._prev=prev;obj._next=prev._next;if(prev._next){prev._next._prev=obj;}else{this.#last=obj;}
...
function displayErrorAtButtons(msg){const errorField=get("errorField");errorField.textContent=msg;errorField.onclick=()=>clearNodes(errorField);}
function isRoTag(tag,owner){return owner.getType().c.some(attr=>(attr.id===tag)&&attr.ro);}
function isFlag(tag,owner){return owner.getType().c.some(attr=>(attr.id===tag)&&((attr.type==="flag")||(attr.type==="numflag")));}
class FilteredMap{#map;#attrs;#lstns;constructor(map,attrs){this.#map=map;this.#attrs=attrs;this.lstns=new Listeners();}
getType(){return this.#attrs;}
toString(obj){return this.#map.toString(obj);}
getName(obj){return this.#map.getName(obj);}
getFullName(obj){return this.#map.getFullName(obj);}
getPath(obj){const path=getPath(this.#attrs);if(obj){if(!obj._type.nameval&&!obj._type.name)return null;return`${path}.${obj._id ?? "new"}`;}
return path;}
getCount(){return null;}
getSize(){return null;}
getObject(id){return this.#map.getObject(id);}
newObject(typeid){return this.#map.newObject(typeid??this.#attrs.ntitle);}
removeObject(obj,cb){this.#map.removeObject(obj,cb);}
getSubtypes(){const types=[];if(this.#map.getSubtypesFor(this.#attrs,types))return types;return null;}
getSubtype(gen,id){return this.#map.getSubtype(gen,id);}
setDefaultConf(obj){return this.#map.setDefaultConf(obj);}
copyConf(obj){return this.#map.copyConf(obj);}
foreach(cb){this.#map.foreach(obj=>{if(this.filter(obj))cb(obj);});}
listen(cb,full){if(this.lstns.listen(cb)){this.lstn=obj=>{if(this.filter(obj))this.lstns.notify(obj);};this.#map.listen(this.lstn,true);}}
unlisten(cb,full){if(this.lstns.unlisten(cb)){this.#map.unlisten(this.lstn,true);}}
filter(obj){for(let type=obj._type;type;type=type.inherit){if(type==this.#attrs)return true;}
return false;}
getNext(obj){return this.#map.getNext(obj);}
moveObjectAfter(obj,after){return this.#map.moveObjectAfter(obj,after);}}
class ObjectAction extends ContainerInterface{#attrs;#lstns;#autorefresh;#req;#rep;constructor(attrs){super();this.#attrs=attrs;this.#lstns=new Listeners();this.#autorefresh=0;this.#req={_type:{c:attrs.request,owner:attrs},_owner:this,};this.#rep={_type:{ro:1,c:attrs.c,owner:attrs},_owner:this,};}
getName(){return getObjectName(this.#req);}
getFullName(){return{name:this.#attrs.title,cfg:getSysProps(this.#attrs),propname:"title",postfix:(this.started?"(Running)":"")};}
getPath(){return getPath(this.#attrs);}
getObject(){return this.getRequest();}
getRequest(){return this.#req;}
getReply(){return this.#rep;}
isRunning(){return this.started;}
start(){this.stop();const req={};update(req,this.#req);req.Uff0001=this.#attrs.path;req.uff0007=this.#attrs.startcmd;for(var i in this.#rep){if(i[0]!='_')delete this.#rep[i];}
post(req,rep=>{if(isError(rep)){showError("Couldn't start",rep);return;}else{notifications.info("Started");}
copyIdTags(this.#req,rep);this.#rep.ufe0001=this.#req.ufe0001;this.#rep.mfe0026=this.#req.mfe0026;assignId(this.#rep);this.started=true;if(this.#autorefresh){this.#lstns.notify(this.#rep);this.#fetch();}else{this.stop();}});}
stop(){if(!this.started)return;this.started=false;this.#clearPrompt();if(this.timer){cancelTimer(this.timer);this.timer=null;}
const req={Uff0001:this.#attrs.path,uff0007:this.#attrs.cancelcmd,};copyIdTags(req,this.#req);post(req,rep=>{this.#lstns.notify(this.#rep)});}#fetch(){const req={Uff0001:this.#attrs.path,uff0007:this.#attrs.pollcmd,};copyIdTags(req,this.#req);if(!this.promtInProgress&&this.confirmAnswer){req.mfe0028=this.confirmAnswer;delete this.confirmAnswer;}
post(req,rep=>{if(!this.started)return;if(isError(rep)){showError("Couldn't continue",rep);this.stop();return;}
update(this.#rep,rep);this.#lstns.notify(this.#rep);if(rep.bfe000b){this.stop();return;}
if(rep.mfe0028){this.#prompt(rep.mfe0028);}else{this.#clearPrompt();}
if(this.#autorefresh&&!this.timer){this.timer=timer(()=>{this.timer=null;this.#fetch();},this.#attrs.autorefresh);}});}#clearPrompt(){this.promptInstace?.close();}#prompt(obj){if(this.promtInProgress){if(this.promtInProgress===obj.s1)return;this.#clearPrompt();}
this.promtInProgress=obj.s1;const fieldset=make.el("fieldset");const legend=make.el("legend",{p:fieldset});legend.innerHTML=obj.s1.replaceAll('\n',"<br>");const group=make.span({p:fieldset,cls:"check-group"});group.style="--columns: 1";obj.S2.forEach((name,idx)=>{const val=obj.S3[idx];const c=make.span({p:group,cls:"check"});const input=make.input({p:c},"radio");input.id=`conformation-${val}`;input.name="promt";input.value=val;make.label({p:c,cont:name},input.id);});this.confirmAnswer={m64:obj,s65:"",}
dialogManager.request(di=>{di.setTitle("Conformation needed");di.onsubmit(e=>{e.preventDefault();const checked=group.querySelector("input:checked");if(checked)this.confirmAnswer.s65=checked.value;di.close();di.onclose(null);});di.onclose(()=>{delete this.promtInProgress;this.timer=null;this.#fetch();});di.setBody([fieldset]);this.promptInstace=di;});}
autostart(obj){this.start();return false;}
listen(cb,full){if(full)++this.#autorefresh;this.#lstns.listen(cb);}
unlisten(cb,full){if(full&&--this.#autorefresh==0){this.stop();}
this.#lstns.unlisten(cb);}}
class queryListeners extends Listeners{#owner;constructor(owner){super();this.#owner=owner;}
listen(cb,full){if(full)this.#owner.listen(null,full);return super.listen(cb);};unlisten(cb,full){if(full)this.#owner.unlisten(null,full);return super.unlisten(cb);};notify(obj){super.notify(obj);};};class ObjectQuery extends ContainerInterface{#attrs;#lstns;#listening;#map;#req;#status;#id;#starting;constructor(attrs){super();this.#attrs=attrs;this.#lstns=new Listeners();this.#listening=0;this.#req={_type:{c:attrs.request,owner:attrs},_owner:this,};if(attrs.status){this.#status={_type:{c:attrs.status,ro:1,owner:attrs},_owner:this,};}
const mapAttrs={c:attrs.c,columns:attrs.columns,path:attrs.path,getallcmd:attrs.getallcmd,ro:1,ordered:attrs.autolimit!=0,autorefresh:attrs.autorefresh,refreshfilter:attrs.refreshfilter,name:attrs.name,nameval:attrs.nameval,generic:attrs.generic,subtypes:attrs.subtypes,typeon:attrs.typeon,owner:attrs};this.#map=new ObjectMap(mapAttrs,new queryListeners(this));this.#map.blockGetall(true);}
getName(){return getObjectName(this.#req);}
getFullName(){return{name:this.#attrs.title,cfg:getSysProps(this.#attrs),propname:"title",postfix:(this.#id!=null?"(Running)":"")};}
getPath(obj){return getPath(this.#attrs);}
getObject(id){if(id==null)return this.getRequest();return this.#map.getObject(id);}
getRequest(){return this.#req;}
getStatus(){return this.#status;}
getMap(){return this.#map;}
isRunning(){return this.#id!=null;}
start(){this.stop();if(this.#starting)return;this.#starting=true;const req={};update(req,this.#req);req.Uff0001=this.#attrs.path;req.uff0007=this.#attrs.startcmd;this.#map.foreach(obj=>this.#map.remove(obj));if(this.#status){for(const i in this.#status){if(i[0]!='_')delete this.#status[i];}}
...
post(req,rep=>{this.#starting=false;if(isError(rep)){showError("Couldn't start",rep);return;}else{notifications.info("Started");}
this.#id=(rep._id!=null)?{_id:rep._id,ufe0001:rep.ufe0001,mfe0026:rep.mfe0026}:{_id:this.#req._id,ufe0001:this.#req.ufe0001,mfe0026:this.#req.mfe0026};if(this.#listening){this.#lstns.notify(this.#req);this.#map.setID(this.#id);this.#map.blockGetall(false);this.#map.getall(rep=>this.onGetall(rep));}else{this.stop();}});}
stop(){if(this.#id!=null){this.#map.blockGetall(true);const req={Uff0001:this.#attrs.path,uff0007:this.#attrs.cancelcmd,};copyIdTags(req,this.#id);this.#id=null;post(req,rep=>this.#lstns.notify(this.#req));}}
onGetall(rep){if(this.#status&&rep.Mfe0002&&rep.Mfe0002.length>=1){update(this.#status,rep.Mfe0002[rep.Mfe0002.length-1]);this.#lstns.notify(this.#status);}
if(this.#id==null)return false;if(isError(rep)){if(rep.uff0008===0xfe0004){return true;}
showError("Couldn't continue",rep);this.stop();return false;}
...
return true;}
autostart(obj){this.start();return false;}
listen(cb,full){if(full)++this.#listening;if(cb)this.#lstns.listen(cb);}
unlisten(cb,full){if(full&&--this.#listening==0){this.stop();}
...
Doit.prototype.getName=function(){return getObjectName(this.obj);};Doit.prototype.getFullName=function(){return{name:this.attrs.title||this.attrs.name,cfg:getSysProps(this.attrs),propname:'title'};};Doit.prototype.getPath=function(){return getPath(this.attrs);};Doit.prototype.getObject=function(){return this.obj;};Doit.prototype.setID=function(obj){if(obj._id!=null){copyIdTags(this.obj,obj);this.obj._id=obj._id;}else{delete this.obj.ufe0001;delete this.obj.mfe0026;delete this.obj._id;}};Doit.prototype.doit=function(cb){const req={};update(req,this.obj);req.Uff0001=this.attrs.path||this.path;req.uff0007=this.attrs.cmd;post(req,rep=>{if(this.attrs.autoreset){update(this.obj,{});this.lstns.notify(this.obj);}
...
function isContainerVisible(c,ignoreNonPublic){return c.title&&c.name!==""&&(ignoreNonPublic||!c.nonpublic)&&(!c.pred||isTrue(c.pred));}
...
CmpOrder.prototype.cmp=function(o1,o2){return o1._num<o2._num;};function CmpAscending(col,coldata,idx,next){this.col=col;this.coldata=coldata;this.idx=idx;this.next=next;this.ascend=true;}
CmpAscending.prototype.cmp=function(o1,o2){var a1=o1!=null?o1._type.columns[this.idx]:null;var a2=o2!=null?o2._type.columns[this.idx]:null;if(this.coldata.attrs.type=="comment")a1=a2=null;if(this.coldata.attrs.type=="deck")a1=a2=this.coldata.deckCont;if(a1==null)a1=this.coldata.attrs;if(a2==null)a2=this.coldata.attrs;var v1=ftype(a1).get(a1,o1);var v2=ftype(a2).get(a2,o2);if(v1==null){if(v2!=null)return true;}else{if(v2==null)return false;const cmpType=(this.coldata.attrs.type=="deck")?this.coldata.deckCont:this.coldata.attrs;if(ftype(cmpType).less(cmpType,v1,v2))return true;if(ftype(cmpType).less(cmpType,v2,v1))return false;}
if(!this.next)return false;return this.next.cmp(o1,o2);};function CmpDescending(col,coldata,idx,next){this.col=col;this.coldata=coldata;this.idx=idx;this.next=next;this.ascend=false;}
CmpDescending.prototype.cmp=function(o1,o2){var a1=o1!=null?o1._type.columns[this.idx]:null;var a2=o2!=null?o2._type.columns[this.idx]:null;if(this.coldata.attrs.type=="comment")a1=a2=null;if(this.coldata.attrs.type=="deck")a1=a2=this.coldata.deckCont;if(a1==null)a1=this.coldata.attrs;if(a2==null)a2=this.coldata.attrs;var v1=ftype(a1).get(a1,o1);var v2=ftype(a2).get(a2,o2);if(v1==null){if(v2!=null)return false;}else{if(v2==null)return true;const cmpType=(this.coldata.attrs.type=="deck")?this.coldata.deckCont:this.coldata.attrs;if(ftype(cmpType).less(cmpType,v2,v1))return true;if(ftype(cmpType).less(cmpType,v1,v2))return false;}
...
class TableCursor{#table;#header;#pointer;#column;#lstns;constructor(table,owner){this.#table=table;this.#header=table.querySelector("thead > tr");this.#pointer=table;this.#column=0;this.#lstns=new Listeners;const navKeys=["ArrowUp","ArrowDown","ArrowLeft","ArrowRight","Home","End","PageUp","PageDown"];this.#table.onkeydown=e=>{if(owner.inMoveMode()){if(e.key==="Escape")get("move-btn")?.click();else if(e.key==="Enter")owner.moveRow(e.target);}
if(!navKeys.includes(e.key))return;e.preventDefault();const cell=cellOf(e.target);const group=e.target.matches("tbody, thead")?e.target:e.target.closest("tbody, thead");const step=5;let nextCell;let last;switch(e.key){case"ArrowUp":case"PageUp":{if(!cell){nextCell=group.previousElementSibling;if(e.key==="PageUp"){for(let i=1;i<step;i++){nextCell=nextCell?.previousElementSibling;}}
nextCell??=this.#table.firstElementChild;break;}
const row=cell.closest("tr");const offset=e.key==="PageUp"?step:1;const idx=row.rowIndex-offset>0?row.rowIndex-offset:0;const nextRow=this.#table.rows[idx];if(nextRow===row)return;nextCell=nextRow.cells[this.#column]??nextRow.lastElementChild;break;}
case"ArrowDown":case"PageDown":{if(!cell){nextCell=group.nextElementSibling;if(e.key==="PageDown"){for(let i=1;i<step;i++){nextCell=nextCell?.nextElementSibling;}}
nextCell??=this.#table.lastElementChild;break;}
const row=cell.closest("tr");const offset=e.key==="PageDown"?step:1;const rows=this.#table.rows.length;const idx=row.rowIndex+offset>rows-1?rows-1:row.rowIndex+offset;const nextRow=this.#table.rows[idx];if(nextRow===row)return;nextCell=nextRow.cells[this.#column]??nextRow.lastElementChild;break;}
case"ArrowLeft":if(!cell){return;}
if(e.altKey)return this.#lstns.notify("shrink",cell);if(this.#focusSibling(e.target,true))return;nextCell=cell.previousElementSibling;if(!nextCell){nextCell=cell.closest("tbody, thead");break;}
this.#column-=1;last=true;break;case"ArrowRight":if(!cell){nextCell=e.target.querySelector("td, th");break;}
if(e.altKey)return this.#lstns.notify("grow",cell);if(this.#focusSibling(e.target))return;nextCell=cell.nextElementSibling;if(!nextCell)return;this.#column+=1;break;case"Home":if(!cell){nextCell=this.#table.firstElementChild;break;}
nextCell=cell.parentElement.cells[0];this.#column=0;if(e.ctrlKey){nextCell=this.#header.cells[0];}
break;case"End":if(!cell){nextCell=this.#table.lastElementChild;break;}
if(e.ctrlKey){const idx=this.#table.rows.length-1
...
this.#column=this.#header.cells.length-1;break;}
this.focus(nextCell,last,true);}}
listen(cb){this.#lstns.listen(cb);}
unlisten(cb){this.#lstns.unlisten(cb);}
focus(cell,last,relative){console.assert(cell.matches("td, th, tbody, thead"),"Focus must be on cell or row");const top=arr=>arr[arr.length-1];if(!cell)return;const oldCell=this.#pointer;this.#pointer=cell??this.#pointer;const focusables=cell.matches("td, th")?getFocusables(this.#pointer):[];const f=(last?top(focusables):focusables[0])??this.#pointer;oldCell.tabIndex=-1;[...oldCell.querySelectorAll("[tabindex='0']")].forEach(e=>e.tabIndex=-1);f.tabIndex=0;f.focus();if(!relative)this.#column=this.#pointer.cellIndex;}#focusSibling(item,prev){const cell=cellOf(item);if(cell===item)return;const focusables=getFocusables(cell);const idx=focusables.indexOf(item);const nextItem=focusables[prev?idx-1:idx+1];if(!nextItem)return;nextItem.tabIndex=0;nextItem.focus();item.tabIndex=-1;return nextItem;}
notifyRemoval(row){if(row.body().contains(this.#pointer))this.focus(this.#table.firstElementChild);}}
class TableRow{#id;#obj;#lstns;#table;#row;#line;#selectBox;#flagCell;#oderCell;#commentCell;#firstRegularCell;#aboutRows;#parent;#children;#level;#treeExpander;constructor(obj,table){if(!("_id"in obj)){this.#level=0;return;}
this.#id=obj._id;this.#obj=obj;this.#lstns=new Listeners();this.#table=table;this.#row=make.el("tbody");this.#row.dataset.id=obj._id;this.#line=make.tr({p:this.#row});this.assignParent(table.dummyParent);const box=make.td({p:this.#line});box.onclick=box.onmousedown=e=>e.stopPropagation();box.onkeydown=e=>{if(e.key===" ")e.stopPropagation();}
box.title="Select row";this.#selectBox=make.input({p:box},"checkbox");this.#selectBox.tabIndex=-1;this.#selectBox.onchange=e=>{if(this.#selectBox.checked){this.#lstns.notify("selected");this.#line.ariaSelected="true";box.title="Unselect row";}else{this.#lstns.notify("deselected");this.#line.ariaSelected="false";box.title="Select row";}
if(e)this.#lstns.notify("organic-sel");}
this.#selectBox.onclick=e=>this.#lstns.notify("check-click",e);this.#flagCell=make.td({p:this.#line});if(table.attrs.ordered){this.#oderCell=make.td({p:this.#line});if(!obj._type.ro)make.div({p:this.#oderCell,cls:"icon icon-tbl icon-Drag_12x12"});make.span({p:this.#oderCell});}
if(table.comment){this.#commentCell=make.td({p:this.#line,cls:"comment"});}
if(table.about)this.#aboutRows=[];for(let i=0;i<table.columns.filter(c=>c).length+1;i++){const cell=make.td({p:this.#line});this.#firstRegularCell??=cell;}
if(this.#firstRegularCell){if(this.#table.treeon){const b=make.button({p:this.#firstRegularCell,cls:"btn"});this.#treeExpander=b;b.tabIndex=-1;make.span({p:b,cls:"icon icon-tbl icon-Minus_12x12"});b.style.visibility="hidden";b.disabled=true;}
if(obj._type.icons??this.#table.attrs.icons){createIcon(this.#firstRegularCell,0);}}}
addHandlers(){const row=this.#row;row.onclick=e=>{if(e.altKey)return this.#table.cursor.focus(cellOf(e.target));if(this.#table.inMoveMode())return this.#lstns.notify("move");if(e.target.matches("input, button, a"))return;this.#lstns.notify("open");}
row.onkeydown=e=>{if(e.key==="Enter"){if(this.#table.inMoveMode())this.#lstns.notify("move");else this.#lstns.notify("open");}
else if(e.key===" ")this.#selectBox.click();}
const getGroup=cell=>cell.closest("tbody, thead");if(this.#table.attrs.ordered&&!this.#table.attrs.ro){if(!this.#table.sortKey)row.draggable=true;const table=this.#table.table;let dragging=null;row.ondragstart=e=>{if(this.#table.inMoveMode())e.preventDefault();dragging=e.target;table.ondragover=e=>{e.preventDefault();hide(dragging);const group=getGroup(e.target);if(!group)return;const fromTop=e.clientY-group.getBoundingClientRect().top;if(fromTop<15){let prevVisible=group.previousElementSibling;if(prevVisible?.hidden)prevVisible=prevVisible.previousElementSibling;prevVisible?.classList.add("droptarget");group.classList.remove("droptarget");}}
table.ondragenter=e=>{const group=getGroup(e.target);if(group!==row)group?.classList.add("droptarget");}
table.ondragleave=e=>{const group=getGroup(e.target);if(group!==getGroup(e.relatedTarget))group.classList.remove("droptarget");}
table.ondrop=e=>{if(!getGroup(e.target).classList.contains("droptarget"))return;this.#lstns.notify("move",getGroup(e.target));}}
row.ondragend=()=>{show(dragging);dragging=null;[...table.children].forEach(r=>r.classList.remove("droptarget"));table.ondragover=null;table.ondragenter=null;table.ondragleave=null;table.ondrop=null;}}
if(this.#treeExpander){this.#treeExpander.onclick=()=>{const set=(row,state)=>{row.getChildren().forEach(r=>{r.body().hidden=state;set(r,state);})};if(this.#row.ariaExpanded==="false"){this.#row.ariaExpanded="true";this.#treeExpander.firstElementChild.classList.replace("icon-Plus_12x12","icon-Minus_12x12");set(this,false);}else{this.#row.ariaExpanded="false";this.#treeExpander.firstElementChild.classList.replace("icon-Minus_12x12","icon-Plus_12x12");set(this,true);}
this.#table.updateStatus();}}}
getId(){return this.#id;}
update(obj){this.#obj=obj;const row=this.#row;const line=this.#line;const flags=this.#flagCell;const flagCont=make.span({cls:"flags"});const bands={};let disabled=false;for(const[attrs,band]of this.#table.flags){if(band&&bands[band])continue;const val=ftype(attrs).get(attrs,obj);const f=ftype(attrs).flag(attrs,val);if(f){if(f[0].className==="disabled")disabled=true;flagCont.appendChild(f[0]);flagCont.title+=f[1]+"\n";if(band)bands[band]=true;}}
if(disabled)row.classList.add("disabled");else row.classList.remove("disabled");if(flags.textContent!==flagCont.textContent){flags.replaceChildren(flagCont);}
if(!flagCont.hasChildNodes())flagCont.remove();const type=obj._type;if(this.#table.attrs.ordered){replaceText(this.#oderCell.lastElementChild,obj._num);}
let cell=this.#firstRegularCell;for(const col of this.#table.columns){if(!col)continue;const colIdx=this.#table.attrs.columns.indexOf(col.attrs);let attrs=type.columns[colIdx];const val=ftype(attrs).get(attrs,obj);attrs=this.#table.attrs.columns[colIdx];if(cell===this.#firstRegularCell){const icons=obj._type.icons??this.#table.attrs.icons;if(icons){let nr=0;if(this.#table.iconon){nr=ftype(this.#table.iconon).get(this.#table.iconon,obj);if(nr==null||this.#table.attrs.icons[nr]==null)nr=0;}
const trueIconNr=icons[nr]||0;const oldIcon=cell.querySelector("[data-nr]");if(Number(oldIcon?.dataset.nr)!==trueIconNr){oldIcon.replaceWith(createIcon(null,trueIconNr));}}}
const data=cell.lastChild?.nodeType===Node.TEXT_NODE?cell.lastChild:null;const d=ftype(attrs).cell(col,obj,val,data);if(data&&d&&!d.isEqualNode(data)){data.replaceWith(d);if(this.#table.treeon&&this.#table.attrs.treeon===col.name){this.changedParent=true;}}
if(data&&!d)data.remove();if(!data&&d)cell.appendChild(d);const c=cell.nextElementSibling;cell=c;}
if(this.#table.comment)this.#updateComment();if(this.#table.about)this.#updateAboutInfo();let color=null;for(const c of this.#table.attrs.colors??[]){if(c.cond==null){const cd=getAttr(this.#table.attrs,c.on);c.cond=cd?new Condition(cd):null;}
if(c.cond?.isTrue(obj)){color=c.color;break;}}
row.style.color=color;}
body(){return this.#row;}
obj(){return this.#obj;}
listen(cb){this.#lstns.listen(cb);}
unlisten(cb){this.#lstns.unlisten(cb);}
assignParent(row){this.#parent?.removeChild(this);row.addChild(this);this.#parent=row;const offset=row.getLevel()+1-this.#level;this.#setLevel(row.getLevel()+1);this.#children?.forEach(c=>c.shiftLevel(offset));}
getParent(){return this.#parent;}#setLevel(level){this.#level=level;this.#row.ariaLevel=String(level);if(this.#table.treeon&&this.#firstRegularCell){this.#firstRegularCell.style.paddingLeft=`${25 * (this.#level - 1)}px`;}}
shiftLevel(offset){this.#setLevel(this.#level+offset);this.#children?.forEach(c=>c.shiftLevel(offset));}#updateChildCount(){this.#children?.forEach(child=>child.setSetSize(this.#children.size));}
setSetSize(size){this.#row.ariaSetSize=size.toString();}
getLevel(){return this.#level;}
addChild(row){this.#children??=new Set;this.#children.add(row);this.#updateChildCount();if(this.#children.size>0&&this.#treeExpander){this.#treeExpander.style.visibility="visible";this.#treeExpander.disabled=false;}}
removeChild(row){this.#children?.delete(row);this.#updateChildCount();if(this.#children?.size===0&&this.#treeExpander){this.#treeExpander.style.visibility="hidden";this.#treeExpander.disabled=true;}}
remove(){this.#row.remove();this.#children?.forEach(c=>{c.assignParent(this.#table.dummyParent)
this.#lstns.notify("orphaned");});}
getChildren(){return[...(this.#children??[])];}#updateComment(){const val=ftype(this.#table.comment).get(this.#table.comment,this.#obj)??"";replaceText(this.#commentCell,val);this.#commentCell.title=val;}#updateAboutInfo(){const val=ftype(this.#table.about).get(this.#table.about,this.#obj)??[];if(val.length===this.#aboutRows.length){this.#aboutRows.forEach((line,idx)=>{replaceText(line.firstChild,val[idx]);});}else{this.#aboutRows.forEach(line=>line.remove());this.#aboutRows.length=0;val.forEach(str=>{const nline=make.tr({cls:"about"});const td=make.td({p:nline,cont:str});td.colSpan=100;this.#row.insertBefore(nline,this.#line);this.#aboutRows.push(nline);});}}}
function Table(cnt,map,sb){this.attrs=map.getType();this.map=map;this.columns=[];this.columnWidths={};this.flags=[];this.rows=new Map();this.order=[];this.selected=new Set();this.selectLstns=new Listeners();this.treeon=!!(this.attrs.treeon??Object.values(this.attrs.subtypes??{}).some(o=>o.treeon));this.orphanRows=new Map();this.dummyParent=new TableRow({},this);this.matcherFilter=[];this.table=get("main-table");this.cursor=new TableCursor(this.table,this);this.header=get("main-table-head");this.destroyLstns=new Listeners();if(this.attrs.iconon)this.iconon=getAttr(this.attrs,this.attrs.iconon);this.dragging=null;this.statusbar=sb;this.splash=new TableSplash(get("main-table")?.parentElement);this.init(cnt);this.lstn=obj=>{if(this.updateRow(obj)){this.updateStatus();this.selectLstns.notify();}};this.map.listen(this.lstn,true);this.update();}
Table.prototype.destroy=function(){if(this.redrawTimer)cancelTimer(this.redrawTimer);this.columns.forEach((col)=>{ftype(col.attrs).unlisten(col.attrs,this.columnLstn);});if(this.filter&&this.filter.unlstn)this.filter.unlstn();this.map.unlisten(this.lstn,true);this.destroyLstns.notify();};Table.prototype.onDestroy=function(cb){this.destroyLstns.listen(cb);};Table.prototype.getPrefs=function(){return preferences.get(getPath(this.attrs));};Table.prototype.setPrefs=function(key,val){preferences.set(key,val,getPath(this.attrs));};Table.prototype.init=function(cnt){function elw(parent,width,title){const e=make.th({p:parent});setCellWidth(e,width);e.title=title;return e;}
this.expired=true;this.columnLstn=()=>{if(!this.expired)return;this.expired=false;this.redrawTimer=timer(()=>{this.redrawTimer=null;this.expired=true;this.update();},200);};this.cursor.listen((event,param)=>{const changeWidth=(cell,offset)=>{const th=this.header.cells[cell.cellIndex];const w=getCellWidth(th)+offset;if(cell.colSpan>1)return;if(th.dataset.name==="comment"){if(offset>0&&this.table.classList.contains("comment-min")){dom.getByClass(this.table,"switch").click();return;}
...
this.initCommentColumn(elw(this.header,107,"Comment"),commentCfg);const addColumn=(col,attrs)=>{if(col&&col.width&&col.width!=0xffffffff){if(this.columns.length==0)col.width+=16;const h=elw(this.header,col.width,col.name);h.classList.add("sort");make.button({p:h,cls:"cell-btn",cont:col.name}).tabIndex=-1;col.attrs=attrs;ftype(attrs).listen(attrs,this.columnLstn);this.columns.push(col);}};this.attrs.columns.forEach((attrs,idx)=>{if((attrs.type==="deck")&&!attrs.nonpublic){var cols=ftype(attrs).columns(attrs,this,this.attrs);cols.forEach(col=>{addColumn(col,attrs);});}else{var col=!attrs.nonpublic?ftype(attrs).column(attrs,this,this.attrs):null;addColumn(col,attrs);}});make.th({p:this.header,cont:'\u00A0'}).width="100%";const addable=!this.attrs.nonaddable||this.attrs.generic;if((addable&&!this.attrs.ro)||this.attrs.removable)show(get("remove-btn"));var header=this.header;[...header.cells].splice(1).forEach(cell=>{const div=make.div();if(cell.previousElementSibling){make.el("i",{p:div,cls:"rsl"}).draggable=true;}
...
this.addFocusHandler();this.loadPrefs();};Table.prototype.initSelectColumn=function(cell){const check=make.input({p:cell},"checkbox");check.tabIndex=-1;check.title="Select all";check.onchange=()=>{const header=this.header;[...this.table.getElementsByTagName("input")].filter(c=>!header.contains(c)).forEach(c=>{c.checked=check.checked;c.onchange();});check.indeterminate=false;this.selectLstns.notify(true);};check.onclick=e=>{if(this.inMoveMode())e.preventDefault();}
...
else if(this.selected.size===this.rows.size){check.checked=true;check.indeterminate=false;}else{check.checked=false;check.indeterminate=true;}
setTitle();})
if(get("controls")){const enable=get("enable-btn");enable.onclick=e=>{this.selected.forEach(row=>{const obj=row.obj();ftype(this.enable).put(this.enable,obj,1);obj._exportObj={bfe000a:false};obj._owner.setObject(obj);})}
const disable=get("disable-btn");disable.onclick=e=>{this.selected.forEach(row=>{const obj=row.obj();ftype(this.enable).put(this.enable,obj,0);obj._exportObj={bfe000a:true};obj._owner.setObject(obj);})}
const remove=get("remove-btn");remove.onclick=e=>{this.selected.forEach(row=>this.map.removeObject(row.obj()));}
const move=get("move-btn");move.onclick=()=>{this.table.classList.toggle("move-mode");move.ariaPressed=this.inMoveMode()?"true":"false";}
this.selectLstns.listen(()=>{if(this.selected.size===0){enable.disabled=true;disable.disabled=true;remove.disabled=true;move.disabled=true;}else{if([...this.selected].some(row=>!isROObj(row.obj())&&!shouldHide(getSysProp(row.obj(),"set"))&&!shouldHide(getSysProp(row.obj(),"enable")))){enable.disabled=false;disable.disabled=false;}
if([...this.selected].some(row=>((!row.obj()._type.nonaddable&&!row.obj()._owner.getType().ro)||row.obj()._owner.getType().removable)&&!shouldHide(getSysProp(row.obj(),"remove")))){remove.disabled=false;}
...
return null;};Table.prototype.update=function(onlyOrder){this.map.foreach(obj=>{this.updateRow(obj,onlyOrder);});this.updateStatus();};Table.prototype.updateStatus=function(){if(!this.statusbar)return;let size=this.table.childElementCount-1;if(this.treeon){[...this.table.children].forEach(child=>{if(child.hidden)size--;});}
if(size===0){this.splash.noEntries();}else{this.splash.hide();}
let msg=size!=1?size+" items":"1 item";const count=this.map.getSize();if(count!=null&&size!=count){msg+=" out of "+count;if(size===0){this.splash.filter();}else{this.splash.hide();}}
...
function cellOf(target){if(!(target instanceof HTMLElement))return;return target.closest("td, th");}
Table.prototype.moveRow=function(cell){const dropTarget=cell.closest("tbody, thead");if(!dropTarget)return;[...this.selected].sort((ra,rb)=>{const a=ra.obj();const b=rb.obj();if(a._num<b._num)return-1;if(a._num>b._num)return 1;return 0;}).forEach(r=>this.map.moveObjectAfter(r.obj(),this.getRowObject(dropTarget)));this.table.classList.remove("move-mode");const move=get("move-btn");move.ariaPressed="false";this.cursor.focus(cell);}
Table.prototype.cellOf=function(element){return element.matches("td, th")?element:element.closest("td, th");}
Table.prototype.addFocusHandler=function(){this.table.onclick=e=>{e.stopPropagation();if(this.inMoveMode()&&!e.altKey){this.moveRow(e.target);return;};const cell=cellOf(e.target);if(!cell)return;if(!cell.contains(document.activeElement))this.cursor.focus(cell);}}
Table.prototype.getRowObject=function(row){const id=row.dataset.id;if(!id)return null;return this.map.getObject(id)??null;};Table.prototype.findRowIndex=function(row){return this.order.indexOf(row);};Table.prototype.findParent=function(obj){if(!obj._type.treeon)return;if(obj.mfe0026){const pattr=this.attrs.c.find(c=>c.name===this.attrs.treeon);const val=ftype(pattr).getvalue(pattr,obj);if(!isempty(val)){obj._parentId=msg2hex(val);return this.order.find(o=>o.getId()===obj._parentId)??obj._parentId;}else{return;}}
if(!obj._parentId){const parentId=obj[obj._type.c.find(a=>a.name===obj._type.treeon).id];if(parentId===undefined)return;obj._parentId=String(parentId);}
return this.order.find(o=>o.getId()===obj._parentId)??obj._parentId;};function hasTrueParent(row,parent,order,idx){if(row.getLevel()===1&&!parent.getId())return true;for(idx--;idx!==-1;idx--){if(order[idx]===parent)return true;if(order[idx].getLevel()>row.getLevel())continue;if(order[idx].getParent()!==parent)return false;}
return false;}
Table.prototype.insertRow=function(row,newRow){const tbody=row.body();const obj=row.obj();const order=this.order;let movedChildren=[];if(!newRow){const isInRightPlace=(sortKey,obj,idx)=>{let prev=order[idx-1];let next=order[idx+1];if(this.treeon){if(row.changedParent){delete row.changedParent;return false;}
const parent=row.getParent();if(parent!==this.dummyParent){if(!hasTrueParent(row,parent,order,idx))return false;if(parent===next)return false;if(parent===prev){prev=null;}else if(parent!==prev?.getParent()&&parent!==next?.getParent()){return false;}}
const siblings=order.filter(r=>parent.getChildren().includes(r));const i=siblings.indexOf(row);prev=siblings[i-1];next=siblings[i+1];}
if(prev&&!sortKey.cmp(prev.obj(),obj)&&sortKey.cmp(obj,prev.obj()))return false;if(next&&!sortKey.cmp(obj,next.obj())&&sortKey.cmp(next.obj(),obj))return false;return true;};const idx=this.findRowIndex(row);if(idx!==-1){if(isInRightPlace(this.sortKey,obj,idx))return;const remainder=order.slice(idx+1);let count=remainder.findIndex(r=>r.getLevel()<=row.getLevel());if(count<0)count=order.length;movedChildren=order.splice(idx+1,count);order.splice(idx,1);}}
let idx;const compare=(a1,a2)=>this.sortKey.cmp(a1,a2);if(this.treeon){let parent=this.findParent(obj);if(typeof parent==="string"){if(!this.orphanRows.has(parent)){this.orphanRows.set(parent,[]);}
this.orphanRows.get(parent).push(row);parent=this.dummyParent;}
parent??=this.dummyParent;row.assignParent(parent);const order=this.order.filter((o,idx,arr)=>o.getParent()===parent&&hasTrueParent(o,parent,arr,idx));idx=binarySearch(order.map(o=>o.obj()),obj,compare);if(idx===order.length){idx=this.order.indexOf(order.pop()??parent)+1;}else{idx=this.order.indexOf(order[idx]);}
const remainder=this.order.slice(idx);const ridx=remainder.findIndex(r=>r.getLevel()<=row.getLevel());if(ridx===-1)idx+=remainder.length;else idx+=ridx;}else{idx=binarySearch(this.order.map(o=>o.obj()),obj,compare);}
const id=obj._id;if(idx<order.length){this.table.insertBefore(tbody,order[idx].body());order.splice(idx,0,this.rows.get(id),...movedChildren);}else{this.table.appendChild(tbody);order.push(this.rows.get(id),...movedChildren);}
tbody.after(...movedChildren.map(r=>r.body()));this.orphanRows.get(id)?.forEach(child=>{const childRow=this.rows.get(child.getId());childRow.assignParent(row);this.insertRow(childRow,false);});this.orphanRows.delete(id);};Table.prototype.insertOrderedRow=function(row,newRow){const id=row.getId();const next=this.getNextRow(row)?.body();const tbody=row.body();if(newRow||tbody.nextSibling!=next){if(!newRow)this.table.removeChild(tbody);if(this.treeon){const parent=this.findParent(row.obj());if(typeof parent==="object"){row.assignParent(parent);}}
if(next){this.table.insertBefore(tbody,next);const idx=this.order.findIndex(row=>row.getId()===next.dataset.id);if(idx>-1){this.order.splice(idx,0,this.rows.get(id));}}else{this.table.appendChild(tbody);this.order.push(this.rows.get(id));}}}
Table.prototype.shouldFold=function(obj){const row=this.rows.get(obj._id);if(!row)return false;for(let r=row.getParent();r.body();r=r.getParent()){if(r.body().ariaExpanded==="false")return true;}
return false;}
Table.prototype.shouldHide=function(obj){for(var i in this.matcherFilter){var match=this.matcherFilter[i];if(!match.func(ftype(match.attr).get(match.attr,obj)))return true;}
if(!this.filter)return false;if(this.filter.value<0){if(this.filter.pred){let val=ftype(this.filter.attr).get(this.filter.attr,obj);return!isTrue(this.filter.pred,val);}
return false;}
const filteron=this.filter.filteron;const val=ftype(filteron).get(filteron,obj);return val!=(filteron.type==="enm"?this.filter.value:this.filter.name);};Table.prototype.getNextRow=function(row){let obj=row.obj();while(obj){obj=this.map.getNext(obj);if(!obj)return null;const row=this.rows.get(obj._id);if(row)return row;}
return null;};Table.prototype.makeRow=function(obj){const row=new TableRow(obj,this);row.addHandlers();this.rows.set(obj._id,row);row.listen((event,param)=>{switch(event){case"selected":this.selected.add(row);break;case"deselected":this.selected.delete(row);break;case"organic-sel":this.selectLstns.notify();break;case"check-click":if(this.inMoveMode())param.preventDefault();break;case"open":openContent(this.map.getPath(obj));break;case"move":if(param){this.map.moveObjectAfter(obj,this.getRowObject(param));}else{this.moveRow(row.body());}
break;case"orphaned":break;}});return row;}
Table.prototype.updateRow=function(obj,onlyOrder){if(obj.ufe0013||this.shouldHide(obj)){this.removeRow(obj);return true;}
...
const id=obj._id;const[newRow,row]=this.rows.has(id)?[false,this.rows.get(id)]:[true,this.makeRow(obj)];if(!onlyOrder||newRow)row.update(obj);if(this.treeon)row.body().hidden=this.shouldFold(obj);if(!this.sortKey){this.insertOrderedRow(row,newRow);}else{this.insertRow(row,newRow);}
return newRow;};Table.prototype.removeRow=function(obj){const id=obj._id;const row=this.rows.get(id);if(!row)return;this.selected.delete(row);this.selectLstns.notify();this.cursor.notifyRemoval(row);this.rows.delete(id);const idx=this.findRowIndex(row);if(idx!==-1)this.order.splice(idx,1);row.remove();row.getChildren().forEach(c=>this.insertRow(c,false));}
Table.prototype.findColumnByName=function(name){var idx=2;if(this.comment)++idx;if(this.attrs.ordered)++idx;for(var i in this.columns){if(this.columns[i]==null)continue;if(this.columns[i].name==name)return this.header.cells[idx];++idx;}
...
const moveBtn=get("move-btn");if(i!=null){const colData=(i==="comment")?{attrs:this.comment}:this.columns[i];const colIdx=(i==="comment")?i:this.attrs.columns.indexOf(colData.attrs);if(ascend){col.classList.add("sort","sort-ascend");col.ariaSort="ascending";this.sortKey=new CmpAscending(col,colData,colIdx,next);}else{col.classList.add("sort","sort-descend");col.ariaSort="descending";this.sortKey=new CmpDescending(col,colData,colIdx,next);}
...
else{for(let i=this.fullSize();i<(sz+this.offset);++i){this.buffer.push({line:"",attr:[]});}}};TermBuf.prototype.erase=function(begin,delCnt){this.buffer.splice(begin,delCnt);};TermBuf.prototype.clearAll=function(){this.offset=0;this.buffer=[];};function Terminal(term){this.term=term;this.path=[76];this.id=null;this.topMargin=0;this.sequence='';this.recvd=0;this.inputpost=false;this.inputbuffer=[];this.deletedRows=0;this.altBuffer=new TermBuf(0,0);this.mainBuffer=new TermBuf(1000,0);this.buf=this.mainBuffer;this.attribute={bold:false,italic:false,underline:false,strikethrough:false,intensity:0,reverse:0,foreground:0,background:15,decoration:function(){let d=[];if(this.underline)d.push('underline');if(this.strikethrough)d.push('line-through');return d.length?d.join(' '):'none';},weight:function(){return this.bold?'bold':'normal';},style:function(){return this.italic?'italic':'normal';}};this.cursor={xpos:0,ypos:0,useGraphic:[false,true],attr:copyObject(this.attribute)};this.savedCursor={xpos:0,ypos:0,useGraphic:[false,true],attr:copyObject(this.attribute)};this.activeCharset=0;this.wrapPending=false;this.insertMode=false;this.keyMode=false;this.arrContainer=[];this.isAutoRepeat=false;this.bufAttrIdx=0;this.hasAttrChanged=false;this.consumer=this.processChar;this.bottomMargin=0;term.onbeforeinput=e=>{e.preventDefault();if(this.failedKeydown){this.input([...e.data].map(c=>c.charCodeAt(0)));this.failedKeydown=false;}}
...
this.repaint();}
return;}
if(e.ctrlKey||e.altKey){if(key=='ArrowLeft'){this.input(e.ctrlKey?this.escSeq("1;5D"):[27,98]);this.ignoreKeyEvent(e);return;}else if(key=='ArrowRight'){this.input(e.ctrlKey?this.escSeq("1;5C"):[27,102]);this.ignoreKeyEvent(e);return;}}
...
this.ensureSpace();var fg=!this.cursor.attr.reverse?this.cursor.attr.foreground:this.cursor.attr.background;var bg=!this.cursor.attr.reverse?this.cursor.attr.background:this.cursor.attr.foreground;if(fg<8){if(this.cursor.attr.intensity){fg+=8;}else if(this.cursor.attr.background!=15){fg+=8;}}
...
var style={}
style.fontWeight=this.cursor.attr.weight();style.fontStyle=this.cursor.attr.style();style.backgroundColor=color(bg);style.color=color(fg);style.textDecoration=this.cursor.attr.decoration();if(attr[x]){for(var i=x+1;i<=this.bufAttrIdx;++i){delete attr[i];}}
attr[x]=style;this.bufAttrIdx=x;this.hasAttrChanged=false;}
this.cursor.xpos++;if(this.cursor.xpos==this.screenWidth){this.cursor.xpos--;this.wrapPending=true;}};Terminal.prototype.selectGraphicRendition=function(args){for(var i=0;i<(args.length||1);++i){var p=this.argAt(args,i,0);switch(p){case 0:this.cursor.attr=copyObject(this.attribute);break;case 1:this.cursor.attr.bold=true;break;case 2:this.cursor.attr.intensity=0;break;case 3:this.cursor.attr.italic=true;break;case 4:this.cursor.attr.underline=true;break;case 5:case 6:break;case 7:this.cursor.attr.reverse=1;break;case 8:break;case 9:this.cursor.attr.strikethrough=true;break;case 21:this.cursor.attr.bold=false;break;case 22:this.cursor.attr.intensity=1;break;case 23:this.cursor.attr.italic=false;break;case 24:this.cursor.attr.underline=false;break;case 25:case 25:break;case 27:this.cursor.attr.reverse=0;break;case 27:break;case 29:this.cursor.attr.strikethrough=false;break;case 38:this.cursor.attr.foreground=0;break;case 39:this.cursor.attr.foreground=0;break;case 49:this.cursor.attr.background=15;break;case 50:this.cursor.attr.background=15;break;default:if(p>=30&&p<=37){this.cursor.attr.foreground=p-30;}else if(p>=40&&p<=47){this.cursor.attr.background=p-40;}
...
if(cfg&&cfg[propname])title=cfg[propname];if(postfix)title+=' '+postfix;setDocumentTitle(title);};Pane.prototype.setTitleFromObject=function(obj,updateOnlyPostfix){var ni=obj._owner.getFullName(obj);this.setTitle(ni.name,ni.cfg,ni.propname,ni.postfix,updateOnlyPostfix);};Pane.prototype.createTabs=function(cont){if(isTabless(cont)||!this.top||cont.hide)return;const tabCnt=get("tabs");dom.addHorizontalNav(tabCnt);const dropBtn=get("tablist");cont.service.c.forEach(c=>{if(!isContainerVisible(c)||c.hide)return;if(!container[c.type].tab)return;const cfg=getContainerProp(c);if(cfg._hide)return;const li=getTemplate("service-tab");tabCnt.appendChild(li);const a=dom.getByTag(li,"a");a.href=getPath(c);a.appendChild(viewLabel(cfg,c.title,"title"));a.title=a.textContent;if(c.title===cont.title){a.classList.add("tabs-active");a.id="active-table-tab";a.setAttribute("aria-selected","true");a.tabIndex=0;dropBtn.textContent=a.title;}});const dropTabCnt=get("dropdown-tabs");dom.addVerticalNav(dropTabCnt);Array.from(tabCnt.children).forEach(t=>{const li=t.cloneNode(true);const a=dom.getByTag(li,"a");if(a.id)a.id="";const active=a.classList.contains("tabs-active")?" active":"";a.className="option"+active;dropTabCnt.appendChild(li);});const dropCnt=dropBtn.parentElement;dropCnt.onclick=e=>{if(e.target!==dropCnt)return;dropCnt.classList.remove("show");dropBtn.ariaExpanded="false";}
...
new Toolbar(get("controls"));const filterSwitch=get("table-filter");if(this.canBeFiltered()){filterSwitch.checked=false;filterSwitch.onclick=e=>{this.toogleFilterMenu();}
filterSwitch.disabled=false;show(filterSwitch);const switchLabel=get("table-filter-label");show(switchLabel);}}
toogleFilterMenu(){this.filters=get("filters");if(!this.filters){}else{this.filters.changed=false;if(this.filters.childNodes.length){if(this.filters.classList.contains("d-none")){this.filters.classList.remove("d-none");}else{this.filters.classList.add("d-none");if(this.table)this.table.matcherFilter=[];this.table?.update();}}else{this.addFilterRow(this.filters);const clearBtn=make.button({p:this.filters.firstElementChild,cont:"Clear",cls:"btn btn-gray"})
clearBtn.onclick=()=>{if(this.table&&this.activeFilters()){this.clearFilters();applyBtn.textContent="Apply";}};const applyBtn=make.button({p:this.filters.firstElementChild,cont:"Apply",cls:"btn btn-dark-green"});applyBtn.type="submit";if(applyBtn.form){applyBtn.form.onchange=e=>{if(this.activeFilters()&&!this.filters.changed){this.filters.changed=true;applyBtn.textContent="Update";}};applyBtn.form.onsubmit=e=>{e.stopPropagation();e.preventDefault();if(this.table){if(this.filters&&this.filters.changed){applyBtn.textContent="Apply";}
this.applyFilters();}}}}}}
addFilterRow(root,before){const fRow=getTemplate("filter-row");fRow.attr=dom.getByClass(fRow,"attribute");fRow.option=dom.getByClass(fRow,"option");fRow.input=dom.getByTag(fRow,"input");fRow.fixedInput=dom.getByClass(fRow,"fixed-value");fRow.plus=dom.getByClass(fRow,"btn-plus");fRow.minus=dom.getByClass(fRow,"btn-minus");fRow.input.classList.add("fixed-value");fRow.attr.onchange=e=>{const prevOpt=fRow.option.value;fRow.option.innerHTML="";const elem=this.attrs.columns[fRow.attr.value];if(!elem)return;const filters=ftype(elem).filters(elem);for(const i in filters){const opt=make.el("option",{p:fRow.option});opt.text=filters[i];opt.value=filters[i];}
for(const i in fRow.option.children){if(fRow.option.children[i].value==prevOpt){fRow.option.selectedIndex=i;break;}}
if(["enable","invalid","dynamic","preset","active","flag","bool","numflag","X_presentflag"].includes(elem.type)){fRow.input.hidden=true;fRow.fixedInput.innerHTML='';make.el("option",{p:fRow.fixedInput,cont:"yes"});make.el("option",{p:fRow.fixedInput,cont:"no"});fRow.fixedInput.hidden=false;}else{fRow.input.hidden=false;fRow.fixedInput.hidden=true;}}
const onEnter=(event)=>{if(event.key==="Enter"){event.currentTarget.form.requestSubmit();}};fRow.attr.onkeydown=onEnter;fRow.option.onkeydown=onEnter;fRow.fixedInput.onkeydown=onEnter;for(const i in this.attrs.columns){const elem=this.attrs.columns[i];if((ftype(elem).filters(elem)?.length>0)&&!elem.nonpublic){const opt=make.el("option");opt.text=(elem.type==="comment")?"Comment":elem.name;opt.value=i;const less=(a,b)=>lexicographicalLess(a.text,b.text);const idx=binarySearch(fRow.attr.children,opt,less);fRow.attr.insertBefore(opt,fRow.attr.childNodes[idx]);}}
fRow.attr.dispatchEvent(new Event('change'));fRow.input.oninput=function(value){fRow.input.style.color='';fRow.input.form.dispatchEvent(new Event('change'));};fRow.plus.onclick=()=>{this.addFilterRow(root,fRow.nextSibling);fRow.plus.form.dispatchEvent(new Event('change'));};fRow.minus.onclick=()=>{const form=fRow.minus.form;this.removeFilterRow(root,fRow);form.dispatchEvent(new Event('change'));};if(before){root.insertBefore(fRow,before);}else{root.appendChild(fRow);}}
removeFilterRow(root,node){if((root.childNodes.length>1)&&root.childNodes[0]!=node){root.removeChild(node);}}
canBeFiltered(){for(const i in this.attrs.columns){const elem=this.attrs.columns[i];if((ftype(elem).filters(elem)?.length>0)&&!elem.nonpublic){return true;}}
return false;}
applyFilters(){if(this.table&&this.filters){this.table.matcherFilter=[];for(var i=0;i<this.filters.childNodes.length;i++){const row=this.filters.childNodes[i];const attr=row.attr.value;const elem=this.attrs.columns[attr];const opt=row.option.value;const valueEntry=row.input;const fixedEntry=row.fixedInput;if(!valueEntry.value&&!valueEntry.hidden)continue;var matcherFunc=ftype(elem).matcher(elem,opt,valueEntry.value);if(valueEntry.hidden){matcherFunc=ftype(elem).matcher(elem,opt,fixedEntry.value);}
if(matcherFunc==null){valueEntry.style.color='red';continue;}
this.table.matcherFilter.push({'func':matcherFunc,'attr':elem});}
this.table.update();this.filters.changed=false;}}
clearFilters(){if(this.table){this.table.matcherFilter=[];this.table.update();if(this.filters)this.filters.changed=false;}}
activeFilters(){if(!this.table)return false;return this.table.matcherFilter?.length>0?true:false;}
addClose(){let path;if(this.attrs.service.owner){const owner=this.attrs.service.owner;path=getPath(findContainer(owner.group,normalize(owner.title)));}else if(this.attrs.hide){path=this.href;}else if(isTabless(this.attrs)){path=getPath(this.attrs.service);}
if(path){super.addCloseButton("Close",null,path);}}
...
const type=this.obj._type;const removeCfg=getSysProp(type,"remove");const copyCfg=getSysProp(type,"copy");const noTables=isTablessContainer(type)||window.location.hash==="#Quick_Set";get("control-buttons")?.replaceWith(getTemplate(`container-${noTables ? "single" : "item"}-controls`));this.newObj=(this.obj._id==null)&&type.type=="map";if(!this.newObj&&type.type=="map"&&((!type.nonaddable&&!type.ro)||type.removable)&&!shouldHide(removeCfg)){this.remove=this.addActionButton("Remove",removeCfg);this.remove.onclick=e=>{this.obj._owner.removeObject(this.obj);openContent(this.href);};}
if(!this.newObj&&(type.type=="map")&&!type.nonaddable&&!type.ro&&!shouldHide(copyCfg)){this.copy=this.addActionButton("Copy",copyCfg);this.copy.onclick=e=>{this.obj._owner.setCopyObj(this.obj);openContent(getPath(this.attrs)+".copy",true);}}
...
create(cnt){this.setTitleFromObject(this.doit.getObject());const ctrlBtns=get("control-buttons");ctrlBtns.replaceWith(getTemplate("container-doit-controls"));const doit=get("DoButton");doit.textContent=getSysProp(this.attrs,"start").name??this.doit.getName();doit.form.onsubmit=e=>e.preventDefault();doit.onclick=e=>{if(!this.ctrl?.save())return;this.ctrl.load();if(this.attrs.confirm&&!confirm(this.attrs.confirm))return;if(this.attrs.progress){this.status.innerHTML=this.attrs.progress;}
this.doit.doit(ok=>{if(ok&&!this.dead)openContent(this.href);});};const close=get("CloseButton");close.textContent=getSysProp(this.attrs,'cancel').name??"Cancel";close.onclick=()=>openContent(this.href);this.ctrl=new ViewController(getContainerProp(this.attrs));this.ctrl.addAllViews(this.doit.getObject());this.ctrl.create(document.getElementById("itemList"));if(this.attrs.progress){const sb=get("statusbar");this.status=make.span({p:sb,cont:"\u00a0"});}}
...
const skinGenerics={};function restructureGeneric(node,parent){if(isContainer(node)){if(!isContainerVisible(node,true)&&parent.type!=="toolbar")return;}else{if(node.nonpublic&&parent)return;}
const irrelevant=["separator","cond","active","columnalias"];const finalContainer=["opt","not","multinumber","multinumberrange","enm","multistring","multiraw","multinetwork","gridcell","tuple","multi","multitristate","union"];if(irrelevant.includes(node.type))return;if(node.type==="contextmenu"&&!node.title&&!node.name)return;if(node.type==="comment")node.name="Comment";const cfg={"type":node.type,"parent":parent,}
...
if(node.request){complexc=complexc.concat(node.request||[]);}
...
return cfg;}
const generalMenuCfg=[];function restructureSysmap(map){const arr=[];arr.push({name:guiStrings.generalMenu,type:"toolbar",node:{name:guiStrings.generalMenu},entries:generalMenuCfg,stuck:true,grouping:true,});arr.push({name:guiStrings.commonNames,type:"toolbar",node:{name:guiStrings.commonNames},entries:[{name:guiStrings.download,type:"button",node:{name:guiStrings.download},stuck:true},{name:guiStrings.upload,type:"button",node:{name:guiStrings.upload},stuck:true},],stuck:true,grouping:true,});arr.push({name:"Terminal",type:"button",node:{name:"Terminal"}});arr.push({name:"Advanced",type:"button",node:{name:"Advanced"}});for(const item of map){if(!isServiceVisible(item,true))continue;const cfg=restructureGeneric(item);cfg.type="service";const group=item.group;if(group){const grpItem=arr.find(e=>e.name===group&&e.type==="serviceGroup");if(!grpItem){arr.push({name:group,type:"serviceGroup",entries:[cfg],node:{name:group}});}else{grpItem.entries.push(cfg);}}else{arr.push(cfg);}}
...
function makeCheckbox(cfg,nameCtrl){const enable=make.input({p:nameCtrl},"checkbox");enable.checked=true;enable.disabled=cfg.node.stuck??false;cfg.getAllChildren=()=>{const allChildren=[];cfg.children?.forEach(child=>{allChildren.push(child);});const buttons=cfg["&"];if(buttons){for(const btn in buttons){allChildren.push(buttons[btn]);}}
return allChildren;}
const updateChecked=node=>{node.checkedChildren=node.getAllChildren().filter(child=>child.ctrls?.show.checked);const noneChecked=(node.checkedChildren.length==0);const allChecked=(node.checkedChildren.length==node.getAllChildren().length);if(!allChecked){if(!node.children.some(child=>!child.hide)&&noneChecked){node.ctrls.show.checked=false;node.hide=true;node.ctrls.show.indeterminate=false;}else{node.ctrls.show.checked=true;delete node.hide;node.ctrls.show.indeterminate=true;}}else{node.ctrls.show.checked=true;delete node.hide;node.ctrls.show.indeterminate=false;}
if(node.parent){updateChecked(node.parent);}}
const checkNode=node=>{node.ctrls.show.checked=true;node.ctrls.show.indeterminate=false;if(node.ctrls.show.disabled)node.ctrls.show.disabled=false;delete node.hide;enableChildrenCtrl(node);}
const uncheckNode=node=>{node.ctrls.show.checked=false;node.ctrls.show.indeterminate=false;if(node.ctrls.show.disabled)node.ctrls.show.disabled=false;node.hide=true;disableChildrenCtrl(node);}
const uncheckChildren=node=>{node.getAllChildren()?.forEach(child=>{uncheckNode(child);});}
const checkChildren=node=>{node.getAllChildren()?.forEach(child=>{checkNode(child);});}
const restoreChildren=node=>{node.getAllChildren()?.forEach(child=>{if(node.checkedChildren?.includes(child)){checkNode(child);}else{uncheckNode(child);}});}
const disableChildrenCtrl=node=>{node.getAllChildren()?.forEach(child=>{child.ctrls.show.disabled=true;disableChildrenCtrl(child);});}
const enableChildrenCtrl=node=>{node.getAllChildren()?.forEach(child=>{child.ctrls.show.disabled=false;enableChildrenCtrl(child);});}
const isRestorableState=node=>{const childCount=node.getAllChildren()?.length;if(childCount==0)return false;if(!node.checkedChildren)return false;if(node.checkedChildren?.length==0)return false;if(node.checkedChildren?.length==childCount)return false;return true;}
const allChildrenEnabled=(node)=>{if(node.getAllChildren().some(child=>!child.ctrls.show.checked))return false;return true;}
const allChildrenDisabled=node=>{if(node.getAllChildren().some(child=>child.ctrls.show.checked))return false;return true;}
enable.onchange=e=>{if(enable.checked){if(isRestorableState(cfg)){restoreChildren(cfg);enable.checked=true;delete cfg.hide;enable.indeterminate=true;}else{checkChildren(cfg);enable.checked=true;delete cfg.hide;}}else{if(!allChildrenEnabled(cfg)){checkChildren(cfg);enable.checked=true;delete cfg.hide;}else{uncheckChildren(cfg);enable.checked=false;cfg.hide=true;}}
if(cfg.parent){updateChecked(cfg.parent);}}
cfg.ctrls.show=enable;}
...
if(isContainer(node)){cfg["&"]=buttonBuilder(table,node,level+1);for(const btn in cfg["&"]){cfg["&"][btn].parent=cfg;}}
...
switch(node.type){case"map":if(!node.ro){buildButton("add","New");buildButton("remove","Remove");buildButton("copy","Copy");buildButton("set","OK");buildButton("apply","Apply");}
...
return cfg;}
function skin2skin(sMap,cfg=skin){for(const key in cfg){const s=sMap.find(s=>key===(isContainer(s.node)?s.origTitle??s.origName:s.origName));const c=cfg[key];if(!s){if(["_hide","name","title","&"].includes(key))continue;let a=sMap.filter(c=>c.node.type==="group");sMap.filter(c=>c.node.type==="tab").forEach(n=>{n.children.filter(c=>c.node.type==="group").forEach(l=>{a=a.concat(l.children);})});a.forEach(e=>skin2skin(e,cfg));continue;}
...
if(buttons[button].name){s["&"][button].ctrls.name.value=buttons[button].name;}}
if(c["&"].name)s["&"].name=c["&"].name;if(c["&"].title)s["&"].title=c["&"].title;}
...
var container={};container.map={};container.map.tab=true;container.map.create=function(cont){return new ObjectMap(cont);};container.map.createPane=function(cont,path,prev){var map=getContainer(cont);if(cont.inherit)map=new FilteredMap(map,cont);if(path.length>=1){var obj;if(path[0]=='new'){obj=map.newObject(path[1]);map.setDefaultConf(obj);}else if(path[0]=='copy'){obj=map.newObject(path[1]);map.copyConf(obj);}else{obj=map.getObject(path[0]);}
...
function isQuickSet(cont){if(cont.service&&cont.service.title=='Quick Set')return true;if(cont.title=='Quick Set')return true;return prevURLs.some(p=>p[1].title==="Quick Set");}
...
function generateMenuBar(){const topCfg=skin[guiStrings.generalMenu]??{};const undoButton=document.getElementById("undo-button");const redoButton=document.getElementById("redo-button");const unCfg=getProp(topCfg,undoButton.title);if(unCfg._hide)hide(undoButton);const updateUndoState=()=>{post({Uff0001:[17],Uff0002:[1],uff0007:0x80007},rep=>{if(isError(rep)){undoButton.disabled=true;if(!redoButton.disabled)redoButton.tabIndex=0;}else{undoButton.disabled=false;redoButton.tabIndex=-1;undoButton.title="Undo: "+rep.s3;undoButton.ariaDescription=undoButton.title;}});}
undoButton.onclick=e=>{post({Uff0001:[17],Uff0002:[1],uff0007:0x80001},rep=>{if(isError(rep))showError("Could not undo",rep);else notifications.info("Undone");},()=>notifications.error("Could not undo"));e.preventDefault();};new Toolbar(get("undo-redo"));const reCfg=getProp(topCfg,redoButton.title);if(reCfg._hide)hide(redoButton);redoButton.onclick=e=>{post({Uff0001:[17],Uff0002:[1],uff0007:0x80002},rep=>{if(isError(rep))showError("Could not redo",rep);else notifications.info("Redone");},()=>notifications.error("Could not redo"));e.preventDefault();};const updateRedoState=()=>{post({Uff0001:[17],Uff0002:[1],uff0007:0x80008},rep=>{if(isError(rep)){redoButton.disabled=true;}
else{redoButton.disabled=false;redoButton.title="Redo: "+rep.s3;redoButton.ariaDescription=redoButton.title;}});}
redoButton.onkeydown=e=>{if(["ArrowLeft","ArrowRight","Home"].includes(e.key))undoButton.focus();}
subscribe([17],rep=>{updateUndoState();updateRedoState();});updateUndoState();updateRedoState();const safeSwitch=document.getElementById("safe-mode");const safeCfg=getProp(topCfg,"Safe Mode");if(safeCfg._hide)hide(safeSwitch.closest("div.check-group"));const bold=safeSwitch.nextElementSibling?.querySelector("b");bold.innerText=viewLabel(safeCfg,"Safe Mode").innerText;safeSwitch.onchange=()=>{safeSwitch.checked=!safeSwitch.checked;toggleSafeMode();};dom.addHorizontalNav(get("app-switch"));function setButton(item,mobileItem){const text=item.lastElementChild.dataset.name;const cfg=getProp(skin,text);if(cfg._hide){hide(item);hide(mobileItem.parentElement);}
const newText=viewLabel(cfg,text).textContent;item.lastElementChild.textContent=newText;mobileItem.textContent=newText;}
setButton(get("id_Quick_Set"),get("id_Quick_Set_alt"));const advanced=get("id_WebFig");const advanced2=get("id_WebFig_alt");const advHandling=()=>{prevURLs=[];openContent(advanced.hash,true);};setButton(advanced,advanced2);advanced.onclick=advHandling;advanced2.onclick=advHandling;setButton(get("id_Terminal"),get("id_Terminal_alt"));generateGeneralMenu();}
...
menu.addEventListener("focusout",e=>{if(menu.contains(e.relatedTarget))return;menu.closest(".btn-select-wrap").classList.remove("show");menu.parentElement.previousElementSibling.ariaExpanded=false;});dom.addVerticalNav(menu);menu.addEventListener("keydown",e=>{if(e.key==="Escape")menuBtn?.focus();});function setButton(item,invisible){const text=item.lastChild.nodeValue;if(invisible==null){if(item.hash){let[a,b]=item.hash.substring(1).split(":");if(b){generalMenuCfg.push({name:text,type:"contextmenu",node:{name:text},contextmenu:{group:a,open:b}});}else{generalMenuCfg.push({name:text,type:"contextmenu",node:{name:text},contextmenu:{open:a}});}}else generalMenuCfg.push({name:text,type:"button",node:{name:text}});}
const cfg=getProp(skin[guiStrings.generalMenu]??{},text);if(invisible??cfg._hide)hide(item.closest("li"));item.lastChild.nodeValue=viewLabel(cfg,text).innerText;}
function setSwitch(item,invisible){const text=item.previousElementSibling.textContent;if(invisible==null)generalMenuCfg.push({name:text,type:"bool",node:{name:text}});const cfg=getProp(skin[guiStrings.generalMenu]??{},text);if(invisible??cfg._hide)hide(item.closest("li"));item.previousElementSibling.textContent=viewLabel(cfg,text).innerText;}
...
class Toolbar{constructor(holder){this.ul=holder;this.reindex();if(holder.getAttribute("aria-orientation")==="vertical"){dom.addVerticalNav(this.ul);}else{dom.addHorizontalNav(this.ul);}}
...
function generateSideMenu(){const menu=get("nav-menu-list");if(menu?.childElementCount>1)return;dom.addVerticalNav(menu);menu.onkeydown=e=>{const item=e.target.closest("li");if(e.key==="ArrowRight"||e.key==="Enter"||e.key===" "){e.preventDefault();e.stopPropagation();item.classList.add("active");dom.getByTag(item,"button").ariaExpanded=true;dom.getByTag(dom.getByClass(item,"level-2").firstElementChild,"a").focus();}}
configSearch.init();for(const menuItem of getSidemenuList(sysmap)){if(menuItem.group){const name=menuItem.name;const normalName=normalize(name);const cfg=getProp(skin,name);if(cfg._hide)continue;const item=getTemplate("side-menu-item-lvl2");const link=item.querySelector("button");link.id=`id_${normalName}`;link.setAttribute("aria-controls",`${link.id}-sublist`);item.querySelector(".icon").classList.add(`icon-${iconName(menuItem.icon)}`);const text=dom.getByClass(item,"text");text.appendChild(viewLabel(cfg,name));text.dataset.name=name;const cleanup=new Cleanup;item.onclick=e=>{if(!item.classList.contains("active")){e.preventDefault();}
item.classList.toggle("active");const btn=dom.getByTag(item,"button");dom.toggleBoolAttribute(btn,"aria-expanded");if(btn.ariaExpanded==="false")return;if(!e.pointerType)subList.firstElementChild?.focus();cleanup.setup(item,btn);};const subList=dom.getByClass(item,"level-2");dom.addVerticalNav(subList);subList.addEventListener("keydown",e=>{if(e.key==="ArrowLeft"){const idx=[...subList.children].findIndex(li=>li.contains(e.target))-10;if(idx<0)subList.previousElementSibling.focus();else dom.getByTag(subList.children[idx],"a").focus();}
if(e.key==="ArrowRight"){const idx=[...subList.children].findIndex(li=>li.contains(e.target))+10;if(idx<subList.childElementCount)dom.getByTag(subList.children[idx],"a").focus();}
if(e.key==="Escape"){subList.previousElementSibling.focus();}
e.stopPropagation();});subList.id=`${link.id}-sublist`;for(let subMenuItem of menuItem.group){if(subMenuItem.name==="Telnet")continue;const name=subMenuItem.name;const prop=getProp(cfg,name);if(prop._hide)continue;const li=getTemplate("side-menu-subitem-lvl2");subList.appendChild(li);const subItem=dom.getByTag(li,"a");subItem.href=getPath(subMenuItem);subItem.appendChild(viewLabel(prop,name));subItem.dataset.name=name;}
subList.addEventListener("focusout",e=>{if(subList.contains(e.relatedTarget))return;subList.closest("li").classList.remove("active");subList.previousElementSibling.ariaExpanded=false;});menu.appendChild(item);continue;}
...
text.appendChild(viewLabel(prop,menuItem.name));text.dataset.name=menuItem.name;menu?.appendChild(lvl1Item);}}
...
const keepAliveMsg={Uff0001:[1],uff0007:0xfe0000}
function keepAlive(){post(keepAliveMsg,rep=>{});}
